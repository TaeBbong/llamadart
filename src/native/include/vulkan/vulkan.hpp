...
GetProcAddress(HINSTANCE hModule, const char *lpProcName);
#endif
#endif
...
public:
  ...
  ArrayWrapper1D(std::string const &data) VULKAN_HPP_NOEXCEPT {
  ...
  template <typename B = T, typename std::enable_if<
                                std::is_same<B, char>::value, int>::type = 0>
  VULKAN_HPP_CONSTEXPR_14
  ArrayWrapper1D(std::string_view data) VULKAN_HPP_NOEXCEPT {
    copy(data.data(), data.length());
  }
#endif

#if (VK_USE_64_BIT_PTR_DEFINES == 0)
  // on 32 bit compiles, needs overloads on index type int to resolve
  // ambiguities
...
private:
  VULKAN_HPP_CONSTEXPR_14 void copy(char const *data,
                                    size_t len) VULKAN_HPP_NOEXCEPT {
...
public:
  ...
  ArrayProxy(uint32_t count, T const *ptr) VULKAN_HPP_NOEXCEPT : m_count(count),
                                                                 m_ptr(ptr) {}

  template <std::size_t C>
  ArrayProxy(T const (&ptr)[C]) VULKAN_HPP_NOEXCEPT : m_count(C), m_ptr(ptr) {}
...
#pragma GCC diagnostic pop
  ...
  // .size() return type implicitly convertible to size_t. The const version can
  // capture temporaries, with lifetime ending at end of statement.
  template <
      typename V,
  ...
  T const *data() const VULKAN_HPP_NOEXCEPT { return m_ptr; }
...
public:
  ...
  template <typename V> ArrayProxyNoTemporaries(V &&value) = delete;
  ...
  template <std::size_t C>
  ArrayProxyNoTemporaries(T (&ptr)[C]) VULKAN_HPP_NOEXCEPT : m_count(C),
                                                             m_ptr(ptr) {}

  template <std::size_t C> ArrayProxyNoTemporaries(T (&&ptr)[C]) = delete;

  template <std::size_t C, typename B = T,
            typename std::enable_if<std::is_const<B>::value, int>::type = 0>
  ArrayProxyNoTemporaries(typename std::remove_const<T>::type (&ptr)[C])
      VULKAN_HPP_NOEXCEPT : m_count(C),
                            m_ptr(ptr) {}
...
template <typename T> class StridedArrayProxy : protected ArrayProxy<T> {
public:
  using ArrayProxy<T>::ArrayProxy;

  StridedArrayProxy(uint32_t count, T const *ptr,
                    uint32_t stride) VULKAN_HPP_NOEXCEPT
  ...
  const T *end() const VULKAN_HPP_NOEXCEPT {
    return reinterpret_cast<T const *>(static_cast<uint8_t const *>(begin()) +
                                       size() * m_stride);
  }
...
public:
  ...
  StructureChain(StructureChain const &rhs) VULKAN_HPP_NOEXCEPT
    ...
    static_assert(StructureChainValidation<sizeof...(ChainElements) - 1,
                                           ChainElements...>::valid,
                  "The structure chain is not valid!");
    link(&std::get<0>(*this), &std::get<0>(rhs),
  ...
  // assign a complete structure to the StructureChain without modifying the
  // chaining
  template <typename T = typename std::tuple_element<
                0, std::tuple<ChainElements...>>::type,
            size_t Which = 0>
...
private:
  ...
  void link(void *dstBase, void const *srcBase, VkBaseOutStructure *dst,
            VkBaseInStructure const *src) {
    while (src->pNext) {
      std::ptrdiff_t offset = reinterpret_cast<char const *>(src->pNext) -
                              reinterpret_cast<char const *>(srcBase);
      dst->pNext = reinterpret_cast<VkBaseOutStructure *>(
          reinterpret_cast<char *>(dstBase) + offset);
      dst = dst->pNext;
      src = src->pNext;
    }
    dst->pNext = nullptr;
...
// interupt the VULKAN_HPP_NAMESPACE for a moment to add specializations of
// std::tuple_size and std::tuple_element for the StructureChain!
}
...
struct tuple_element<Index, VULKAN_HPP_NAMESPACE::StructureChain<Elements...>> {
  using type =
      typename std::tuple_element<Index, std::tuple<Elements...>>::type;
};
} // namespace std
...
public:
  ...
  VkResult vkGetPhysicalDeviceToolProperties(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkGetPhysicalDeviceToolProperties(physicalDevice, pToolCount,
                                               pToolProperties);
  }
  ...
  VkResult vkCreatePrivateDataSlot(
    ...
    return ::vkCreatePrivateDataSlot(device, pCreateInfo, pAllocator,
                                     pPrivateDataSlot);
  }
  ...
  VkResult vkSetPrivateData(VkDevice device, VkObjectType objectType,
                            uint64_t objectHandle,
                            VkPrivateDataSlot privateDataSlot,
                            uint64_t data) const VULKAN_HPP_NOEXCEPT {
    return ::vkSetPrivateData(device, objectType, objectHandle, privateDataSlot,
                              data);
  }

  void vkGetPrivateData(VkDevice device, VkObjectType objectType,
                        uint64_t objectHandle,
                        VkPrivateDataSlot privateDataSlot,
                        uint64_t *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPrivateData(device, objectType, objectHandle, privateDataSlot,
                              pData);
  }
  ...
  void vkCmdWaitEvents2(VkCommandBuffer commandBuffer, uint32_t eventCount,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdWaitEvents2(commandBuffer, eventCount, pEvents,
                              pDependencyInfos);
  }
  ...
  VkResult vkQueueSubmit2(VkQueue queue, uint32_t submitCount,
  ...
  void vkCmdSetViewportWithCount(
      ...
      const VkViewport *pViewports) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetViewportWithCount(commandBuffer, viewportCount,
                                       pViewports);
  }
  ...
  void vkCmdBindVertexBuffers2(
    ...
    return ::vkCmdBindVertexBuffers2(commandBuffer, firstBinding, bindingCount,
                                     pBuffers, pOffsets, pSizes, pStrides);
  }

  void
  vkCmdSetDepthTestEnable(VkCommandBuffer commandBuffer,
                          VkBool32 depthTestEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetStencilOp(VkCommandBuffer commandBuffer,
    ...
    return ::vkCmdSetStencilOp(commandBuffer, faceMask, failOp, passOp,
  ...
  void vkCmdSetRasterizerDiscardEnable(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetRasterizerDiscardEnable(commandBuffer,
                                             rasterizerDiscardEnable);
  }

  void
  vkCmdSetDepthBiasEnable(VkCommandBuffer commandBuffer,
                          VkBool32 depthBiasEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetPrimitiveRestartEnable(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetPrimitiveRestartEnable(commandBuffer,
  ...
  void vkGetDeviceBufferMemoryRequirements(
      ...
      VkMemoryRequirements2 *pMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceBufferMemoryRequirements(device, pInfo,
                                                 pMemoryRequirements);
  }
  ...
  void vkGetDeviceImageMemoryRequirements(
      ...
      VkMemoryRequirements2 *pMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceImageMemoryRequirements(device, pInfo,
                                                pMemoryRequirements);
  }
  ...
  VkResult vkCreateVideoSessionKHR(
    ...
    return ::vkCreateVideoSessionKHR(device, pCreateInfo, pAllocator,
                                     pVideoSession);
  }
  ...
  VkResult vkBindVideoSessionMemoryKHR(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkBindVideoSessionMemoryKHR(device, videoSession,
  ...
  VkResult vkCreateVideoSessionParametersKHR(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCreateVideoSessionParametersKHR(device, pCreateInfo, pAllocator,
  ...
  void vkCmdEndRenderingKHR(VkCommandBuffer commandBuffer) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetDiscardRectangleEnableEXT(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetDiscardRectangleEnableEXT(commandBuffer,
  ...
  VkResult vkCreateExecutionGraphPipelinesAMDX(
      ...
      const VkAllocationCallbacks *pAllocator,
      VkPipeline *pPipelines) const VULKAN_HPP_NOEXCEPT {
    return ::vkCreateExecutionGraphPipelinesAMDX(device, pipelineCache,
  ...
  VkResult vkGetExecutionGraphPipelineScratchSizeAMDX(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkGetExecutionGraphPipelineScratchSizeAMDX(device, executionGraph,
                                                        pSizeInfo);
  }
  ...
  VkResult vkGetExecutionGraphPipelineNodeIndexAMDX(
    ...
    return ::vkGetExecutionGraphPipelineNodeIndexAMDX(device, executionGraph,
  ...
  void vkCmdDispatchGraphIndirectCountAMDX(
    ...
    return ::vkCmdDispatchGraphIndirectCountAMDX(commandBuffer, scratch,
                                                 countInfo);
  }
  ...
  VkResult vkWaitForPresentKHR(VkDevice device, VkSwapchainKHR swapchain,
                               uint64_t presentId,
                               uint64_t timeout) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkGetImageSubresourceLayout2EXT(
    ...
    return ::vkGetImageSubresourceLayout2EXT(device, image, pSubresource,
                                             pLayout);
  }
  ...
  VkResult
  vkAcquireDrmDisplayEXT(VkPhysicalDevice physicalDevice, int32_t drmFd,
                         VkDisplayKHR display) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetEncodedVideoSessionParametersKHR(
      ...
      size_t *pDataSize, void *pData) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult
  vkCreateCudaModuleNV(VkDevice device,
  ...
  VkResult vkGetCudaModuleCacheNV(VkDevice device, VkCudaModuleNV module,
                                  size_t *pCacheSize,
  ...
  VkResult vkCreateCudaFunctionNV(
      VkDevice device, const VkCudaFunctionCreateInfoNV *pCreateInfo,
      const VkAllocationCallbacks *pAllocator,
  ...
  void vkDestroyCudaModuleNV(VkDevice device, VkCudaModuleNV module,
                             const VkAllocationCallbacks *pAllocator) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkDestroyCudaModuleNV(device, module, pAllocator);
  }

  void vkDestroyCudaFunctionNV(VkDevice device, VkCudaFunctionNV function,
                               const VkAllocationCallbacks *pAllocator) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdCudaLaunchKernelNV(VkCommandBuffer commandBuffer,
                               const VkCudaLaunchInfoNV *pLaunchInfo) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkExportMetalObjectsEXT(VkDevice device,
  ...
  void vkGetDescriptorSetLayoutSizeEXT(
      ...
      VkDeviceSize *pLayoutSizeInBytes) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDescriptorSetLayoutSizeEXT(device, layout,
                                             pLayoutSizeInBytes);
  }
  ...
  void vkGetDescriptorSetLayoutBindingOffsetEXT(
    ...
    return ::vkGetDescriptorSetLayoutBindingOffsetEXT(device, layout, binding,
                                                      pOffset);
  }
  ...
  void vkCmdBindDescriptorBuffersEXT(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBindDescriptorBuffersEXT(commandBuffer, bufferCount,
                                           pBindingInfos);
  }
  ...
  void vkCmdBindDescriptorBufferEmbeddedSamplersEXT(
      VkCommandBuffer commandBuffer, VkPipelineBindPoint pipelineBindPoint,
      VkPipelineLayout layout, uint32_t set) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetImageOpaqueCaptureDescriptorDataEXT(
      VkDevice device, const VkImageCaptureDescriptorDataInfoEXT *pInfo,
      void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetImageOpaqueCaptureDescriptorDataEXT(device, pInfo, pData);
  }

  VkResult vkGetImageViewOpaqueCaptureDescriptorDataEXT(
      VkDevice device, const VkImageViewCaptureDescriptorDataInfoEXT *pInfo,
      void *pData) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetSamplerOpaqueCaptureDescriptorDataEXT(
      VkDevice device, const VkSamplerCaptureDescriptorDataInfoEXT *pInfo,
      void *pData) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdDrawMeshTasksEXT(VkCommandBuffer commandBuffer,
                             uint32_t groupCountX, uint32_t groupCountY,
                             uint32_t groupCountZ) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawMeshTasksEXT(commandBuffer, groupCountX, groupCountY,
                                   groupCountZ);
  }

  void
  vkCmdDrawMeshTasksIndirectEXT(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                VkDeviceSize offset, uint32_t drawCount,
                                uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawMeshTasksIndirectEXT(commandBuffer, buffer, offset,
                                           drawCount, stride);
  }
  ...
  void vkCmdDrawMeshTasksIndirectCountEXT(
      ...
      VkBuffer countBuffer, VkDeviceSize countBufferOffset,
      uint32_t maxDrawCount, uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawMeshTasksIndirectCountEXT(commandBuffer, buffer, offset,
                                                countBuffer, countBufferOffset,
                                                maxDrawCount, stride);
  }
  ...
  VkResult vkCreateBufferCollectionFUCHSIA(
    ...
    return ::vkCreateBufferCollectionFUCHSIA(device, pCreateInfo, pAllocator,
                                             pCollection);
  }
  ...
  VkResult vkGetDeviceSubpassShadingMaxWorkgroupSizeHUAWEI(
    ...
    return ::vkGetDeviceSubpassShadingMaxWorkgroupSizeHUAWEI(device, renderpass,
                                                             pMaxWorkgroupSize);
  }

  void vkCmdSubpassShadingHUAWEI(VkCommandBuffer commandBuffer) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdBindInvocationMaskHUAWEI(
      VkCommandBuffer commandBuffer, VkImageView imageView,
      VkImageLayout imageLayout) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBindInvocationMaskHUAWEI(commandBuffer, imageView,
                                           imageLayout);
  }
  ...
  VkResult vkGetMemoryRemoteAddressNV(
    ...
    return ::vkGetMemoryRemoteAddressNV(device, pMemoryGetRemoteAddressInfo,
                                        pAddress);
  }
  ...
  VkResult vkGetPipelinePropertiesEXT(
    ...
    return ::vkGetPipelinePropertiesEXT(device, pPipelineInfo,
                                        pPipelineProperties);
  }
  ...
  void vkCmdSetRasterizerDiscardEnableEXT(
    ...
    return ::vkCmdSetRasterizerDiscardEnableEXT(commandBuffer,
                                                rasterizerDiscardEnable);
  }
  ...
  void vkCmdSetLogicOpEXT(VkCommandBuffer commandBuffer,
                          VkLogicOp logicOp) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetPrimitiveRestartEnableEXT(VkCommandBuffer commandBuffer,
                                         VkBool32 primitiveRestartEnable) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetPrimitiveRestartEnableEXT(commandBuffer,
                                               primitiveRestartEnable);
  }
  ...
  void vkCmdSetColorWriteEnableEXT(
    ...
    return ::vkCmdSetColorWriteEnableEXT(commandBuffer, attachmentCount,
                                         pColorWriteEnables);
  }
  ...
  void vkCmdDrawMultiEXT(VkCommandBuffer commandBuffer, uint32_t drawCount,
                         ...
                         uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawMultiEXT(commandBuffer, drawCount, pVertexInfo,
  ...
  void vkCmdDrawMultiIndexedEXT(
    ...
    return ::vkCmdDrawMultiIndexedEXT(commandBuffer, drawCount, pIndexInfo,
                                      instanceCount, firstInstance, stride,
                                      pVertexOffset);
  }
  ...
  VkResult vkWriteMicromapsPropertiesEXT(
      VkDevice device, uint32_t micromapCount, const VkMicromapEXT *pMicromaps,
      VkQueryType queryType, size_t dataSize, void *pData,
      size_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkWriteMicromapsPropertiesEXT(device, micromapCount, pMicromaps,
  ...
  void vkCmdWriteMicromapsPropertiesEXT(
      ...
      VkQueryPool queryPool, uint32_t firstQuery) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdWriteMicromapsPropertiesEXT(commandBuffer, micromapCount,
  ...
  void vkGetDeviceMicromapCompatibilityEXT(
      ...
      VkAccelerationStructureCompatibilityKHR *pCompatibility) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceMicromapCompatibilityEXT(device, pVersionInfo,
                                                 pCompatibility);
  }
  ...
  void vkGetMicromapBuildSizesEXT(
    ...
    return ::vkGetMicromapBuildSizesEXT(device, buildType, pBuildInfo,
                                        pSizeInfo);
  }
  ...
  void vkCmdDrawClusterHUAWEI(VkCommandBuffer commandBuffer,
                              uint32_t groupCountX, uint32_t groupCountY,
                              uint32_t groupCountZ) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawClusterHUAWEI(commandBuffer, groupCountX, groupCountY,
                                    groupCountZ);
  }
  ...
  void vkSetDeviceMemoryPriorityEXT(VkDevice device, VkDeviceMemory memory,
                                    float priority) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkGetDeviceBufferMemoryRequirementsKHR(
      VkDevice device, const VkDeviceBufferMemoryRequirements *pInfo,
      VkMemoryRequirements2 *pMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceBufferMemoryRequirementsKHR(device, pInfo,
                                                    pMemoryRequirements);
  }
  ...
  void vkGetDeviceImageMemoryRequirementsKHR(
      VkDevice device, const VkDeviceImageMemoryRequirements *pInfo,
      VkMemoryRequirements2 *pMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceImageMemoryRequirementsKHR(device, pInfo,
                                                   pMemoryRequirements);
  }
  ...
  void vkCmdCopyMemoryIndirectNV(VkCommandBuffer commandBuffer,
                                 ...
                                 uint32_t copyCount,
                                 uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdCopyMemoryIndirectNV(commandBuffer, copyBufferAddress,
                                       copyCount, stride);
  }
  ...
  void vkCmdCopyMemoryToImageIndirectNV(
    ...
    return ::vkCmdCopyMemoryToImageIndirectNV(
        commandBuffer, copyBufferAddress, copyCount, stride, dstImage,
  ...
  void vkCmdDecompressMemoryNV(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDecompressMemoryNV(commandBuffer, decompressRegionCount,
  ...
  void vkGetPipelineIndirectMemoryRequirementsNV(
      ...
      VkMemoryRequirements2 *pMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPipelineIndirectMemoryRequirementsNV(device, pCreateInfo,
                                                       pMemoryRequirements);
  }
  ...
  void vkCmdUpdatePipelineIndirectBufferNV(
      VkCommandBuffer commandBuffer, VkPipelineBindPoint pipelineBindPoint,
      VkPipeline pipeline) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdUpdatePipelineIndirectBufferNV(commandBuffer,
  ...
  vkCmdSetPolygonModeEXT(VkCommandBuffer commandBuffer,
                         VkPolygonMode polygonMode) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetRasterizationSamplesEXT(
    ...
    return ::vkCmdSetRasterizationSamplesEXT(commandBuffer,
                                             rasterizationSamples);
  }

  void vkCmdSetSampleMaskEXT(
      VkCommandBuffer commandBuffer, VkSampleCountFlagBits samples,
      const VkSampleMask *pSampleMask) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetAlphaToCoverageEnableEXT(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetAlphaToCoverageEnableEXT(commandBuffer,
  ...
  vkCmdSetLogicOpEnableEXT(VkCommandBuffer commandBuffer,
                           VkBool32 logicOpEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetColorBlendEnableEXT(
    ...
    return ::vkCmdSetColorBlendEnableEXT(commandBuffer, firstAttachment,
  ...
  void vkCmdSetColorWriteMaskEXT(
    ...
    return ::vkCmdSetColorWriteMaskEXT(commandBuffer, firstAttachment,
  ...
  void vkCmdSetLineRasterizationModeEXT(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetLineRasterizationModeEXT(commandBuffer,
  ...
  void vkCmdSetDepthClipNegativeOneToOneEXT(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetDepthClipNegativeOneToOneEXT(commandBuffer,
                                                  negativeOneToOne);
  }
  ...
  void vkCmdSetViewportWScalingEnableNV(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetViewportWScalingEnableNV(commandBuffer,
  ...
  void vkCmdSetViewportSwizzleNV(
    ...
    return ::vkCmdSetViewportSwizzleNV(commandBuffer, firstViewport,
  ...
  void vkCmdSetCoverageToColorEnableNV(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetCoverageToColorEnableNV(commandBuffer,
  ...
  void vkCmdSetCoverageModulationModeNV(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetCoverageModulationModeNV(commandBuffer,
  ...
  void vkCmdSetShadingRateImageEnableNV(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetShadingRateImageEnableNV(commandBuffer,
  ...
  void vkCmdSetCoverageReductionModeNV(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetCoverageReductionModeNV(commandBuffer,
  ...
  void vkGetShaderModuleCreateInfoIdentifierEXT(
      VkDevice device, const VkShaderModuleCreateInfo *pCreateInfo,
      VkShaderModuleIdentifierEXT *pIdentifier) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetShaderModuleCreateInfoIdentifierEXT(device, pCreateInfo,
                                                      pIdentifier);
  }
  ...
  VkResult vkCreateOpticalFlowSessionNV(
    ...
    return ::vkCreateOpticalFlowSessionNV(device, pCreateInfo, pAllocator,
                                          pSession);
  }
  ...
  VkResult vkBindOpticalFlowSessionImageNV(
    ...
    return ::vkBindOpticalFlowSessionImageNV(device, session, bindingPoint,
                                             view, layout);
  }
  ...
  void
  vkCmdBindIndexBuffer2KHR(VkCommandBuffer commandBuffer, VkBuffer buffer,
                           VkDeviceSize offset, VkDeviceSize size,
                           VkIndexType indexType) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBindIndexBuffer2KHR(commandBuffer, buffer, offset, size,
                                      indexType);
  }
  ...
  void vkGetRenderingAreaGranularityKHR(
    ...
    return ::vkGetRenderingAreaGranularityKHR(device, pRenderingAreaInfo,
                                              pGranularity);
  }
  ...
  void vkGetImageSubresourceLayout2KHR(
      ...
      const VkImageSubresource2KHR *pSubresource,
      VkSubresourceLayout2KHR *pLayout) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetImageSubresourceLayout2KHR(device, image, pSubresource,
                                             pLayout);
  }
  ...
  VkResult vkCreateShadersEXT(VkDevice device, uint32_t createInfoCount,
    ...
    return ::vkCreateShadersEXT(device, createInfoCount, pCreateInfos,
                                pAllocator, pShaders);
  }
  ...
  VkResult vkGetShaderBinaryDataEXT(VkDevice device, VkShaderEXT shader,
                                    size_t *pDataSize,
                                    void *pData) const VULKAN_HPP_NOEXCEPT {
  ...
  void
  vkCmdBindShadersEXT(VkCommandBuffer commandBuffer, uint32_t stageCount,
  ...
  VkResult vkGetFramebufferTilePropertiesQCOM(
    ...
    return ::vkGetFramebufferTilePropertiesQCOM(device, framebuffer,
  ...
  VkResult vkGetDynamicRenderingTilePropertiesQCOM(
      ...
      VkTilePropertiesQCOM *pProperties) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDynamicRenderingTilePropertiesQCOM(device, pRenderingInfo,
                                                     pProperties);
  }
  ...
  void vkCmdSetAttachmentFeedbackLoopEnableEXT(
      VkCommandBuffer commandBuffer,
      VkImageAspectFlags aspectMask) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetScreenBufferPropertiesQNX(
      VkDevice device, const struct _screen_buffer *buffer,
  ...
  void vkCmdBindDescriptorSets2KHR(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBindDescriptorSets2KHR(commandBuffer,
  ...
  void vkCmdSetDescriptorBufferOffsets2EXT(
      ...
      const VkSetDescriptorBufferOffsetsInfoEXT
          *pSetDescriptorBufferOffsetsInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdBindDescriptorBufferEmbeddedSamplers2EXT(
      ...
      const VkBindDescriptorBufferEmbeddedSamplersInfoEXT *
          pBindDescriptorBufferEmbeddedSamplersInfo) const VULKAN_HPP_NOEXCEPT {
...
public:
  ...
  ObjectRelease(OwnerType owner, Dispatch const &dispatch
                                     VULKAN_HPP_DEFAULT_DISPATCHER_ASSIGNMENT)
      VULKAN_HPP_NOEXCEPT : m_owner(owner),
                            m_dispatch(&dispatch) {}
...
#else
    return VULKAN_HPP_NAMESPACE::to_string(
        static_cast<VULKAN_HPP_NAMESPACE::Result>(ev));
#endif
  }
};
...
public:
  ImageUsageNotSupportedKHRError(std::string const &message)
      : SystemError(make_error_code(Result::eErrorImageUsageNotSupportedKHR),
                    message) {}
...
public:
  VideoPictureLayoutNotSupportedKHRError(std::string const &message)
      : SystemError(
            make_error_code(Result::eErrorVideoPictureLayoutNotSupportedKHR),
            message) {}
...
public:
  VideoProfileOperationNotSupportedKHRError(std::string const &message)
      : SystemError(
            make_error_code(Result::eErrorVideoProfileOperationNotSupportedKHR),
            message) {}
...
public:
  VideoProfileFormatNotSupportedKHRError(std::string const &message)
      : SystemError(
            make_error_code(Result::eErrorVideoProfileFormatNotSupportedKHR),
            message) {}
...
public:
  VideoProfileCodecNotSupportedKHRError(std::string const &message)
      : SystemError(
            make_error_code(Result::eErrorVideoProfileCodecNotSupportedKHR),
            message) {}
...
public:
  VideoStdVersionNotSupportedKHRError(std::string const &message)
      : SystemError(
            make_error_code(Result::eErrorVideoStdVersionNotSupportedKHR),
            message) {}
...
public:
  InvalidVideoStdParametersKHRError(std::string const &message)
      : SystemError(make_error_code(Result::eErrorInvalidVideoStdParametersKHR),
                    message) {}
...
public:
  CompressionExhaustedEXTError(std::string const &message)
      : SystemError(make_error_code(Result::eErrorCompressionExhaustedEXT),
                    message) {}
...
public:
  IncompatibleShaderBinaryEXTError(std::string const &message)
      : SystemError(make_error_code(Result::eErrorIncompatibleShaderBinaryEXT),
                    message) {}
...
namespace detail {
[[noreturn]] VULKAN_HPP_INLINE void throwResultException(Result result,
                                                         char const *message) {
  switch (result) {
  ...
  case Result::eErrorVideoPictureLayoutNotSupportedKHR:
    throw VideoPictureLayoutNotSupportedKHRError(message);
  ...
  case Result::eErrorVideoProfileFormatNotSupportedKHR:
    throw VideoProfileFormatNotSupportedKHRError(message);
  case Result::eErrorVideoProfileCodecNotSupportedKHR:
    throw VideoProfileCodecNotSupportedKHRError(message);
  case Result::eErrorVideoStdVersionNotSupportedKHR:
    throw VideoStdVersionNotSupportedKHRError(message);
  ...
  case Result::eErrorNotPermittedKHR:
    throw NotPermittedKHRError(message);
#if defined(VK_USE_PLATFORM_WIN32_KHR)
  ...
  case Result::eErrorInvalidVideoStdParametersKHR:
    throw InvalidVideoStdParametersKHRError(message);
  case Result::eErrorCompressionExhaustedEXT:
    throw CompressionExhaustedEXTError(message);
  case Result::eErrorIncompatibleShaderBinaryEXT:
    throw IncompatibleShaderBinaryEXTError(message);
  default:
...
struct ResultValue<UniqueHandle<Type, Dispatch>> {
  ...
  VULKAN_HPP_DEPRECATED(
      "asTuple() on an l-value is deprecated, as it implicitly moves the "
      "UniqueHandle out of the ResultValue. Use asTuple() on an r-value "
      "instead, requiring to explicitly move the UniqueHandle.")
...
template <typename T,
          typename = typename std::enable_if<std::is_integral<T>::value>::type>
VULKAN_HPP_CONSTEXPR uint32_t makeApiVersion(T const variant, T const major,
                                             T const minor, T const patch) {
...
template <typename T,
          typename = typename std::enable_if<std::is_integral<T>::value>::type>
VULKAN_HPP_DEPRECATED(
    "This define is deprecated. VK_MAKE_API_VERSION should be used instead.")
VULKAN_HPP_CONSTEXPR uint32_t
    makeVersion(T const major, T const minor, T const patch) {
  return ((((uint32_t)(major)) << 22U) | (((uint32_t)(minor)) << 12U) |
          ((uint32_t)(patch)));
}
...
template <typename T,
          typename = typename std::enable_if<std::is_integral<T>::value>::type>
VULKAN_HPP_DEPRECATED(
    "This define is deprecated. VK_API_VERSION_MAJOR should be used instead.")
...
template <typename T,
          typename = typename std::enable_if<std::is_integral<T>::value>::type>
VULKAN_HPP_DEPRECATED(
    "This define is deprecated. VK_API_VERSION_MINOR should be used instead.")
...
template <typename T,
          typename = typename std::enable_if<std::is_integral<T>::value>::type>
VULKAN_HPP_DEPRECATED(
    "This define is deprecated. VK_API_VERSION_PATCH should be used instead.")
...
VULKAN_HPP_CONSTEXPR_INLINE auto HeaderVersionComplete =
    makeApiVersion(0, 1, 3, VK_HEADER_VERSION);
...
#if defined(VK_USE_PLATFORM_XLIB_KHR)
//=== VK_KHR_xlib_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRXlibSurfaceExtensionName =
    VK_KHR_XLIB_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRXlibSurfaceSpecVersion =
    VK_KHR_XLIB_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_XLIB_KHR*/

#if defined(VK_USE_PLATFORM_XCB_KHR)
//=== VK_KHR_xcb_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRXcbSurfaceExtensionName =
    VK_KHR_XCB_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRXcbSurfaceSpecVersion =
    VK_KHR_XCB_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_XCB_KHR*/

#if defined(VK_USE_PLATFORM_WAYLAND_KHR)
//=== VK_KHR_wayland_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRWaylandSurfaceExtensionName =
    VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRWaylandSurfaceSpecVersion =
    VK_KHR_WAYLAND_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_WAYLAND_KHR*/

#if defined(VK_USE_PLATFORM_ANDROID_KHR)
//=== VK_KHR_android_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRAndroidSurfaceExtensionName =
    VK_KHR_ANDROID_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRAndroidSurfaceSpecVersion =
    VK_KHR_ANDROID_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_ANDROID_KHR*/

#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_KHR_win32_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRWin32SurfaceExtensionName =
    VK_KHR_WIN32_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRWin32SurfaceSpecVersion =
    VK_KHR_WIN32_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_WIN32_KHR*/

//=== VK_EXT_debug_report ===
VULKAN_HPP_DEPRECATED("The VK_EXT_debug_report extension has been deprecated "
                      "by VK_EXT_debug_utils.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_sampler_mirror_clamp_to_edge extension has "
                      "been promoted to core in version 1.2.")
...
//=== VK_EXT_debug_marker ===
VULKAN_HPP_DEPRECATED("The VK_EXT_debug_marker extension has been promoted to "
                      "VK_EXT_debug_utils.")
...
VULKAN_HPP_DEPRECATED("The VK_NV_dedicated_allocation extension has been "
...
//=== VK_AMD_draw_indirect_count ===
VULKAN_HPP_DEPRECATED("The VK_AMD_draw_indirect_count extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_AMD_negative_viewport_height extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_AMD_gpu_shader_half_float extension has been "
...
//=== VK_KHR_dynamic_rendering ===
VULKAN_HPP_DEPRECATED("The VK_KHR_dynamic_rendering extension has been "
...
#if defined(VK_USE_PLATFORM_GGP)
//=== VK_GGP_stream_descriptor_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto GGPStreamDescriptorSurfaceExtensionName =
    VK_GGP_STREAM_DESCRIPTOR_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto GGPStreamDescriptorSurfaceSpecVersion =
...
//=== VK_KHR_multiview ===
VULKAN_HPP_DEPRECATED(
    "The VK_KHR_multiview extension has been promoted to core in version 1.1.")
...
//=== VK_NV_external_memory_capabilities ===
VULKAN_HPP_DEPRECATED("The VK_NV_external_memory_capabilities extension has "
                      "been deprecated by VK_KHR_external_memory_capabilities.")
...
VULKAN_HPP_DEPRECATED("The VK_NV_external_memory extension has been deprecated "
                      "by VK_KHR_external_memory.")
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_NV_external_memory_win32 ===
VULKAN_HPP_DEPRECATED("The VK_NV_external_memory_win32 extension has been "
                      "deprecated by VK_KHR_external_memory_win32.")
VULKAN_HPP_CONSTEXPR_INLINE auto NVExternalMemoryWin32ExtensionName =
    VK_NV_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME;
VULKAN_HPP_DEPRECATED("The VK_NV_external_memory_win32 extension has been "
                      "deprecated by VK_KHR_external_memory_win32.")
VULKAN_HPP_CONSTEXPR_INLINE auto NVExternalMemoryWin32SpecVersion =
    VK_NV_EXTERNAL_MEMORY_WIN32_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_WIN32_KHR*/

#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_NV_win32_keyed_mutex ===
VULKAN_HPP_DEPRECATED("The VK_NV_win32_keyed_mutex extension has been promoted "
                      "to VK_KHR_win32_keyed_mutex.")
VULKAN_HPP_CONSTEXPR_INLINE auto NVWin32KeyedMutexExtensionName =
    VK_NV_WIN32_KEYED_MUTEX_EXTENSION_NAME;
VULKAN_HPP_DEPRECATED("The VK_NV_win32_keyed_mutex extension has been promoted "
                      "to VK_KHR_win32_keyed_mutex.")
VULKAN_HPP_CONSTEXPR_INLINE auto NVWin32KeyedMutexSpecVersion =
    VK_NV_WIN32_KEYED_MUTEX_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_WIN32_KHR*/

//=== VK_KHR_get_physical_device_properties2 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_get_physical_device_properties2 extension "
                      "has been promoted to core in version 1.1.")
...
//=== VK_KHR_device_group ===
VULKAN_HPP_DEPRECATED("The VK_KHR_device_group extension has been promoted to "
                      "core in version 1.1.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_validation_flags extension has been "
...
#if defined(VK_USE_PLATFORM_VI_NN)
//=== VK_NN_vi_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto NNViSurfaceExtensionName =
    VK_NN_VI_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NNViSurfaceSpecVersion =
    VK_NN_VI_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_VI_NN*/

//=== VK_KHR_shader_draw_parameters ===
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_draw_parameters extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_EXT_shader_subgroup_ballot extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_EXT_shader_subgroup_vote extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_EXT_texture_compression_astc_hdr extension has "
                      "been promoted to core in version 1.3.")
...
//=== VK_KHR_maintenance1 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_maintenance1 extension has been promoted to "
                      "core in version 1.1.")
...
//=== VK_KHR_device_group_creation ===
VULKAN_HPP_DEPRECATED("The VK_KHR_device_group_creation extension has been "
                      "promoted to core in version 1.1.")
...
//=== VK_KHR_external_memory_capabilities ===
VULKAN_HPP_DEPRECATED("The VK_KHR_external_memory_capabilities extension has "
                      "been promoted to core in version 1.1.")
...
//=== VK_KHR_external_memory ===
VULKAN_HPP_DEPRECATED("The VK_KHR_external_memory extension has been promoted "
                      "to core in version 1.1.")
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_KHR_external_memory_win32 ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRExternalMemoryWin32ExtensionName =
    VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRExternalMemoryWin32SpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRWin32KeyedMutexExtensionName =
    VK_KHR_WIN32_KEYED_MUTEX_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRWin32KeyedMutexSpecVersion =
    VK_KHR_WIN32_KEYED_MUTEX_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_WIN32_KHR*/

//=== VK_KHR_external_semaphore_capabilities ===
VULKAN_HPP_DEPRECATED("The VK_KHR_external_semaphore_capabilities extension "
                      "has been promoted to core in version 1.1.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_external_semaphore extension has been "
                      "promoted to core in version 1.1.")
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_KHR_external_semaphore_win32 ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRExternalSemaphoreWin32ExtensionName =
    VK_KHR_EXTERNAL_SEMAPHORE_WIN32_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRExternalSemaphoreWin32SpecVersion =
...
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_float16_int8 extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_KHR_16bit_storage extension has been promoted to "
                      "core in version 1.1.")
...
//=== VK_KHR_descriptor_update_template ===
VULKAN_HPP_DEPRECATED("The VK_KHR_descriptor_update_template extension has "
                      "been promoted to core in version 1.1.")
...
VULKAN_HPP_CONSTEXPR_INLINE auto EXTAcquireXlibDisplaySpecVersion =
...
VULKAN_HPP_DEPRECATED("The VK_KHR_imageless_framebuffer extension has been "
                      "promoted to core in version 1.2.")
...
//=== VK_KHR_create_renderpass2 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_create_renderpass2 extension has been "
                      "promoted to core in version 1.2.")
...
//=== VK_KHR_external_fence_capabilities ===
VULKAN_HPP_DEPRECATED("The VK_KHR_external_fence_capabilities extension has "
                      "been promoted to core in version 1.1.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_external_fence extension has been promoted "
                      "to core in version 1.1.")
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_KHR_external_fence_win32 ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRExternalFenceWin32ExtensionName =
    VK_KHR_EXTERNAL_FENCE_WIN32_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRExternalFenceWin32SpecVersion =
...
//=== VK_KHR_maintenance2 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_maintenance2 extension has been promoted to "
                      "core in version 1.1.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_variable_pointers extension has been "
                      "promoted to core in version 1.1.")
...
#if defined(VK_USE_PLATFORM_IOS_MVK)
//=== VK_MVK_ios_surface ===
VULKAN_HPP_DEPRECATED("The VK_MVK_ios_surface extension has been deprecated by "
                      "VK_EXT_metal_surface.")
VULKAN_HPP_CONSTEXPR_INLINE auto MVKIosSurfaceExtensionName =
    VK_MVK_IOS_SURFACE_EXTENSION_NAME;
VULKAN_HPP_DEPRECATED("The VK_MVK_ios_surface extension has been deprecated by "
                      "VK_EXT_metal_surface.")
VULKAN_HPP_CONSTEXPR_INLINE auto MVKIosSurfaceSpecVersion =
    VK_MVK_IOS_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_IOS_MVK*/

#if defined(VK_USE_PLATFORM_MACOS_MVK)
//=== VK_MVK_macos_surface ===
VULKAN_HPP_DEPRECATED("The VK_MVK_macos_surface extension has been deprecated "
                      "by VK_EXT_metal_surface.")
VULKAN_HPP_CONSTEXPR_INLINE auto MVKMacosSurfaceExtensionName =
    VK_MVK_MACOS_SURFACE_EXTENSION_NAME;
VULKAN_HPP_DEPRECATED("The VK_MVK_macos_surface extension has been deprecated "
                      "by VK_EXT_metal_surface.")
VULKAN_HPP_CONSTEXPR_INLINE auto MVKMacosSurfaceSpecVersion =
...
VULKAN_HPP_DEPRECATED("The VK_KHR_dedicated_allocation extension has been "
                      "promoted to core in version 1.1.")
...
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
//=== VK_ANDROID_external_memory_android_hardware_buffer ===
VULKAN_HPP_CONSTEXPR_INLINE auto
    ANDROIDExternalMemoryAndroidHardwareBufferExtensionName =
        VK_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto
    ANDROIDExternalMemoryAndroidHardwareBufferSpecVersion =
        VK_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_ANDROID_KHR*/

//=== VK_EXT_sampler_filter_minmax ===
VULKAN_HPP_DEPRECATED("The VK_EXT_sampler_filter_minmax extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_storage_buffer_storage_class extension has "
                      "been promoted to core in version 1.1.")
...
VULKAN_HPP_DEPRECATED("The VK_AMD_gpu_shader_int16 extension has been "
                      "deprecated by VK_KHR_shader_float16_int8.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_inline_uniform_block extension has been "
                      "promoted to core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_relaxed_block_layout extension has been "
                      "promoted to core in version 1.1.")
...
//=== VK_KHR_get_memory_requirements2 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_get_memory_requirements2 extension has been "
                      "promoted to core in version 1.1.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_image_format_list extension has been "
                      "promoted to core in version 1.2.")
...
//=== VK_KHR_sampler_ycbcr_conversion ===
VULKAN_HPP_DEPRECATED("The VK_KHR_sampler_ycbcr_conversion extension has been "
                      "promoted to core in version 1.1.")
...
//=== VK_KHR_bind_memory2 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_bind_memory2 extension has been promoted to "
                      "core in version 1.1.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_descriptor_indexing extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_shader_viewport_index_layer extension has "
                      "been promoted to core in version 1.2.")
...
//=== VK_KHR_maintenance3 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_maintenance3 extension has been promoted to "
                      "core in version 1.1.")
...
//=== VK_KHR_draw_indirect_count ===
VULKAN_HPP_DEPRECATED("The VK_KHR_draw_indirect_count extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_global_priority extension has been promoted "
                      "to VK_KHR_global_priority.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_subgroup_extended_types extension has "
                      "been promoted to core in version 1.2.")
...
//=== VK_KHR_8bit_storage ===
VULKAN_HPP_DEPRECATED("The VK_KHR_8bit_storage extension has been promoted to "
                      "core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_atomic_int64 extension has been "
                      "promoted to core in version 1.2.")
...
//=== VK_EXT_calibrated_timestamps ===
VULKAN_HPP_DEPRECATED("The VK_EXT_calibrated_timestamps extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_EXT_vertex_attribute_divisor extension has been "
...
#if defined(VK_USE_PLATFORM_GGP)
//=== VK_GGP_frame_token ===
VULKAN_HPP_CONSTEXPR_INLINE auto GGPFrameTokenExtensionName =
    VK_GGP_FRAME_TOKEN_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto GGPFrameTokenSpecVersion =
    VK_GGP_FRAME_TOKEN_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_GGP*/

//=== VK_EXT_pipeline_creation_feedback ===
VULKAN_HPP_DEPRECATED("The VK_EXT_pipeline_creation_feedback extension has "
                      "been promoted to core in version 1.3.")
...
//=== VK_KHR_driver_properties ===
VULKAN_HPP_DEPRECATED("The VK_KHR_driver_properties extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_float_controls extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_depth_stencil_resolve extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_NV_fragment_shader_barycentric extension has "
                      "been promoted to VK_KHR_fragment_shader_barycentric.")
...
//=== VK_KHR_timeline_semaphore ===
VULKAN_HPP_DEPRECATED("The VK_KHR_timeline_semaphore extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_vulkan_memory_model extension has been "
                      "promoted to core in version 1.2.")
...
#if defined(VK_USE_PLATFORM_FUCHSIA)
//=== VK_FUCHSIA_imagepipe_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto FUCHSIAImagepipeSurfaceExtensionName =
    VK_FUCHSIA_IMAGEPIPE_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto FUCHSIAImagepipeSurfaceSpecVersion =
    VK_FUCHSIA_IMAGEPIPE_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_FUCHSIA*/

//=== VK_KHR_shader_terminate_invocation ===
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_terminate_invocation extension has "
                      "been promoted to core in version 1.3.")
...
#if defined(VK_USE_PLATFORM_METAL_EXT)
//=== VK_EXT_metal_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTMetalSurfaceExtensionName =
    VK_EXT_METAL_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTMetalSurfaceSpecVersion =
...
VULKAN_HPP_DEPRECATED("The VK_EXT_scalar_block_layout extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_subgroup_size_control extension has been "
                      "promoted to core in version 1.3.")
...
//=== VK_KHR_spirv_1_4 ===
VULKAN_HPP_DEPRECATED(
    "The VK_KHR_spirv_1_4 extension has been promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_separate_depth_stencil_layouts extension has "
                      "been promoted to core in version 1.2.")
...
//=== VK_EXT_buffer_device_address ===
VULKAN_HPP_DEPRECATED("The VK_EXT_buffer_device_address extension has been "
...
//=== VK_EXT_tooling_info ===
VULKAN_HPP_DEPRECATED("The VK_EXT_tooling_info extension has been promoted to "
                      "core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_separate_stencil_usage extension has been "
                      "promoted to core in version 1.2.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_validation_features extension has been "
                      "deprecated by VK_EXT_layer_settings.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_uniform_buffer_standard_layout extension has "
                      "been promoted to core in version 1.2.")
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_EXT_full_screen_exclusive ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTFullScreenExclusiveExtensionName =
    VK_EXT_FULL_SCREEN_EXCLUSIVE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTFullScreenExclusiveSpecVersion =
...
//=== VK_KHR_buffer_device_address ===
VULKAN_HPP_DEPRECATED("The VK_KHR_buffer_device_address extension has been "
                      "promoted to core in version 1.2.")
...
//=== VK_EXT_host_query_reset ===
VULKAN_HPP_DEPRECATED("The VK_EXT_host_query_reset extension has been promoted "
                      "to core in version 1.2.")
...
//=== VK_EXT_extended_dynamic_state ===
VULKAN_HPP_DEPRECATED("The VK_EXT_extended_dynamic_state extension has been "
                      "promoted to core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_shader_demote_to_helper_invocation extension "
                      "has been promoted to core in version 1.3.")
VULKAN_HPP_CONSTEXPR_INLINE auto
...
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_integer_dot_product extension has "
                      "been promoted to core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_texel_buffer_alignment extension has been "
                      "promoted to core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_shader_non_semantic_info extension has been "
                      "promoted to core in version 1.3.")
...
//=== VK_EXT_private_data ===
VULKAN_HPP_DEPRECATED("The VK_EXT_private_data extension has been promoted to "
                      "core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_pipeline_creation_cache_control extension "
                      "has been promoted to core in version 1.3.")
...
#if defined(VK_USE_PLATFORM_METAL_EXT)
//=== VK_EXT_metal_objects ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTMetalObjectsExtensionName =
    VK_EXT_METAL_OBJECTS_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTMetalObjectsSpecVersion =
    VK_EXT_METAL_OBJECTS_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_METAL_EXT*/

//=== VK_KHR_synchronization2 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_synchronization2 extension has been promoted "
                      "to core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_KHR_zero_initialize_workgroup_memory extension "
                      "has been promoted to core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_ycbcr_2plane_444_formats extension has been "
                      "promoted to core in version 1.3.")
...
VULKAN_HPP_DEPRECATED("The VK_EXT_image_robustness extension has been promoted "
                      "to core in version 1.3.")
...
//=== VK_KHR_copy_commands2 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_copy_commands2 extension has been promoted "
                      "to core in version 1.3.")
...
//=== VK_EXT_4444_formats ===
VULKAN_HPP_DEPRECATED("The VK_EXT_4444_formats extension has been promoted to "
                      "core in version 1.3.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_ARM_rasterization_order_attachment_access extension has been "
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
//=== VK_NV_acquire_winrt_display ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVAcquireWinrtDisplayExtensionName =
    VK_NV_ACQUIRE_WINRT_DISPLAY_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVAcquireWinrtDisplaySpecVersion =
    VK_NV_ACQUIRE_WINRT_DISPLAY_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_WIN32_KHR*/

#if defined(VK_USE_PLATFORM_DIRECTFB_EXT)
//=== VK_EXT_directfb_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTDirectfbSurfaceExtensionName =
    VK_EXT_DIRECTFB_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTDirectfbSurfaceSpecVersion =
    VK_EXT_DIRECTFB_SURFACE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_DIRECTFB_EXT*/

//=== VK_VALVE_mutable_descriptor_type ===
VULKAN_HPP_DEPRECATED("The VK_VALVE_mutable_descriptor_type extension has been "
...
VULKAN_HPP_DEPRECATED("The VK_KHR_format_feature_flags2 extension has been "
                      "promoted to core in version 1.3.")
...
VULKAN_HPP_CONSTEXPR_INLINE auto FUCHSIAExternalMemorySpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto FUCHSIAExternalSemaphoreSpecVersion =
    VK_FUCHSIA_EXTERNAL_SEMAPHORE_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_FUCHSIA*/

#if defined(VK_USE_PLATFORM_FUCHSIA)
//=== VK_FUCHSIA_buffer_collection ===
VULKAN_HPP_CONSTEXPR_INLINE auto FUCHSIABufferCollectionExtensionName =
    VK_FUCHSIA_BUFFER_COLLECTION_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto FUCHSIABufferCollectionSpecVersion =
...
//=== VK_EXT_extended_dynamic_state2 ===
VULKAN_HPP_DEPRECATED("The VK_EXT_extended_dynamic_state2 extension has been "
                      "promoted to core in version 1.3.")
...
#if defined(VK_USE_PLATFORM_SCREEN_QNX)
//=== VK_QNX_screen_surface ===
VULKAN_HPP_CONSTEXPR_INLINE auto QNXScreenSurfaceExtensionName =
    VK_QNX_SCREEN_SURFACE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto QNXScreenSurfaceSpecVersion =
...
//=== VK_EXT_global_priority_query ===
VULKAN_HPP_DEPRECATED("The VK_EXT_global_priority_query extension has been "
...
//=== VK_KHR_maintenance4 ===
VULKAN_HPP_DEPRECATED("The VK_KHR_maintenance4 extension has been promoted to "
                      "core in version 1.3.")
...
VULKAN_HPP_CONSTEXPR_INLINE auto ANDROIDExternalFormatResolveExtensionName =
    VK_ANDROID_EXTERNAL_FORMAT_RESOLVE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto ANDROIDExternalFormatResolveSpecVersion =
...
#if defined(VK_USE_PLATFORM_SCREEN_QNX)
//=== VK_QNX_external_memory_screen_buffer ===
VULKAN_HPP_CONSTEXPR_INLINE auto QNXExternalMemoryScreenBufferExtensionName =
    VK_QNX_EXTERNAL_MEMORY_SCREEN_BUFFER_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto QNXExternalMemoryScreenBufferSpecVersion =
...
template <>
struct StructExtends<PipelineLayoutCreateInfo,
...
template <>
struct StructExtends<PhysicalDeviceVulkan13Properties,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PipelineCreationFeedbackCreateInfo,
...
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
template <>
struct StructExtends<PhysicalDeviceShaderTerminateInvocationFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceShaderDemoteToHelperInvocationFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDevicePrivateDataFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDevicePipelineCreationCacheControlFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<MemoryBarrier2, SubpassDependency2> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceSynchronization2Features,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceZeroInitializeWorkgroupMemoryFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceImageRobustnessFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceSubgroupSizeControlFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceSubgroupSizeControlProperties,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PipelineShaderStageRequiredSubgroupSizeCreateInfo,
...
template <>
struct StructExtends<PhysicalDeviceInlineUniformBlockFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceInlineUniformBlockProperties,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
template <>
struct StructExtends<DescriptorPoolInlineUniformBlockCreateInfo,
                     DescriptorPoolCreateInfo> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceTextureCompressionASTCHDRFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<CommandBufferInheritanceRenderingInfo,
                     CommandBufferInheritanceInfo> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceShaderIntegerDotProductFeatures,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceShaderIntegerDotProductProperties,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
template <> struct StructExtends<FormatProperties3, FormatProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceMaintenance4Features,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceMaintenance4Properties,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_KHR_video_queue ===
template <>
struct StructExtends<QueueFamilyQueryResultStatusPropertiesKHR,
...
template <> struct StructExtends<VideoProfileInfoKHR, QueryPoolCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoProfileListInfoKHR,
                     PhysicalDeviceVideoFormatInfoKHR> {
  enum { value = true };
};

template <> struct StructExtends<VideoProfileListInfoKHR, ImageCreateInfo> {
  enum { value = true };
};

template <> struct StructExtends<VideoProfileListInfoKHR, BufferCreateInfo> {
  enum { value = true };
};
...
template <> struct StructExtends<VideoDecodeUsageInfoKHR, VideoProfileInfoKHR> {
  enum { value = true };
};

template <> struct StructExtends<VideoDecodeUsageInfoKHR, QueryPoolCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeH264QualityLevelPropertiesKHR,
                     VideoEncodeQualityLevelPropertiesKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH264SessionCreateInfoKHR,
                     VideoSessionCreateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH264SessionParametersCreateInfoKHR,
                     VideoSessionParametersCreateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH264SessionParametersAddInfoKHR,
                     VideoSessionParametersUpdateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH264SessionParametersGetInfoKHR,
                     VideoEncodeSessionParametersGetInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH264SessionParametersFeedbackInfoKHR,
...
template <>
struct StructExtends<VideoEncodeH264RateControlInfoKHR,
...
template <>
struct StructExtends<VideoEncodeH264RateControlLayerInfoKHR,
                     VideoEncodeRateControlLayerInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH264GopRemainingFrameInfoKHR,
                     VideoBeginCodingInfoKHR> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeH265SessionCreateInfoKHR,
                     VideoSessionCreateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH265QualityLevelPropertiesKHR,
                     VideoEncodeQualityLevelPropertiesKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH265SessionParametersCreateInfoKHR,
                     VideoSessionParametersCreateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH265SessionParametersAddInfoKHR,
                     VideoSessionParametersUpdateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH265SessionParametersGetInfoKHR,
                     VideoEncodeSessionParametersGetInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoEncodeH265SessionParametersFeedbackInfoKHR,
                     VideoEncodeSessionParametersFeedbackInfoKHR> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeH265RateControlInfoKHR,
                     VideoCodingControlInfoKHR> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeH265GopRemainingFrameInfoKHR,
                     VideoBeginCodingInfoKHR> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoDecodeH264SessionParametersCreateInfoKHR,
                     VideoSessionParametersCreateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoDecodeH264SessionParametersAddInfoKHR,
                     VideoSessionParametersUpdateInfoKHR> {
  enum { value = true };
};
...
//=== VK_KHR_dynamic_rendering ===
template <>
struct StructExtends<RenderingFragmentShadingRateAttachmentInfoKHR,
                     RenderingInfo> {
  enum { value = true };
};

template <>
struct StructExtends<RenderingFragmentDensityMapAttachmentInfoEXT,
                     RenderingInfo> {
  enum { value = true };
};

template <>
struct StructExtends<AttachmentSampleCountInfoAMD,
                     CommandBufferInheritanceInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<MultiviewPerViewAttributesInfoNVX,
                     CommandBufferInheritanceInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDevicePipelineRobustnessPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PipelineRobustnessCreateInfoEXT,
                     GraphicsPipelineCreateInfo> {
  enum { value = true };
};
...
//=== VK_IMG_relaxed_line_rasterization ===
template <>
struct StructExtends<PhysicalDeviceRelaxedLineRasterizationFeaturesIMG,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<PerformanceQuerySubmitInfoKHR, SubmitInfo2> {
  enum { value = true };
};
...
template <>
struct StructExtends<DebugUtilsObjectNameInfoEXT,
                     PipelineShaderStageCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<AndroidHardwareBufferFormatProperties2ANDROID,
                     AndroidHardwareBufferPropertiesANDROID> {
  enum { value = true };
};
#endif /*VK_USE_PLATFORM_ANDROID_KHR*/
...
//=== VK_AMDX_shader_enqueue ===
template <>
struct StructExtends<PhysicalDeviceShaderEnqueueFeaturesAMDX,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceShaderEnqueuePropertiesAMDX,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PipelineShaderStageNodeCreateInfoAMDX,
                     PipelineShaderStageCreateInfo> {
  enum { value = true };
};
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
...
template <> struct StructExtends<SampleLocationsInfoEXT, ImageMemoryBarrier2> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoDecodeH265SessionParametersCreateInfoKHR,
                     VideoSessionParametersCreateInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<VideoDecodeH265SessionParametersAddInfoKHR,
                     VideoSessionParametersUpdateInfoKHR> {
  enum { value = true };
};
...
//=== VK_KHR_global_priority ===
template <>
struct StructExtends<DeviceQueueGlobalPriorityCreateInfoKHR,
                     DeviceQueueCreateInfo> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceGlobalPriorityQueryFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<QueueFamilyGlobalPriorityPropertiesKHR,
                     QueueFamilyProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_provoking_vertex ===
template <>
struct StructExtends<PhysicalDeviceProvokingVertexFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceProvokingVertexPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_host_image_copy ===
template <>
struct StructExtends<PhysicalDeviceHostImageCopyFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceHostImageCopyPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
template <>
struct StructExtends<HostImageCopyDevicePerformanceQueryEXT,
                     ImageFormatProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_shader_atomic_float2 ===
template <>
struct StructExtends<PhysicalDeviceShaderAtomicFloat2FeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<SurfacePresentScalingCapabilitiesEXT,
                     SurfaceCapabilities2KHR> {
  enum { value = true };
};

template <>
struct StructExtends<SurfacePresentModeCompatibilityEXT,
                     SurfaceCapabilities2KHR> {
  enum { value = true };
};
...
//=== VK_EXT_swapchain_maintenance1 ===
template <>
struct StructExtends<PhysicalDeviceSwapchainMaintenance1FeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<SwapchainPresentFenceInfoEXT, PresentInfoKHR> {
  enum { value = true };
};
...
template <> struct StructExtends<SwapchainPresentModeInfoEXT, PresentInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<SwapchainPresentScalingCreateInfoEXT,
                     SwapchainCreateInfoKHR> {
  enum { value = true };
};
...
//=== VK_NV_inherited_viewport_scissor ===
template <>
struct StructExtends<PhysicalDeviceInheritedViewportScissorFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<CommandBufferInheritanceViewportScissorInfoNV,
                     CommandBufferInheritanceInfo> {
  enum { value = true };
};
...
//=== VK_EXT_depth_bias_control ===
template <>
struct StructExtends<PhysicalDeviceDepthBiasControlFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<DepthBiasRepresentationInfoEXT,
                     PipelineRasterizationStateCreateInfo> {
  enum { value = true };
};
...
//=== VK_NV_present_barrier ===
template <>
struct StructExtends<PhysicalDevicePresentBarrierFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<SurfaceCapabilitiesPresentBarrierNV,
                     SurfaceCapabilities2KHR> {
  enum { value = true };
};

template <>
struct StructExtends<SwapchainPresentBarrierCreateInfoNV,
                     SwapchainCreateInfoKHR> {
  enum { value = true };
};

//=== VK_KHR_present_id ===
template <> struct StructExtends<PresentIdKHR, PresentInfoKHR> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDevicePresentIdFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<QueryPoolVideoEncodeFeedbackCreateInfoKHR,
                     QueryPoolCreateInfo> {
  enum { value = true };
};

template <> struct StructExtends<VideoEncodeUsageInfoKHR, VideoProfileInfoKHR> {
  enum { value = true };
};

template <> struct StructExtends<VideoEncodeUsageInfoKHR, QueryPoolCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeQualityLevelInfoKHR,
                     VideoCodingControlInfoKHR> {
  enum { value = true };
};
...
//=== VK_NV_cuda_kernel_launch ===
template <>
struct StructExtends<PhysicalDeviceCudaKernelLaunchFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceCudaKernelLaunchPropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
...
template <> struct StructExtends<ImportMetalBufferInfoEXT, MemoryAllocateInfo> {
  enum { value = true };
};
...
template <> struct StructExtends<ImportMetalTextureInfoEXT, ImageCreateInfo> {
  enum { value = true };
};
...
template <> struct StructExtends<ImportMetalIOSurfaceInfoEXT, ImageCreateInfo> {
  enum { value = true };
};
...
//=== VK_EXT_descriptor_buffer ===
template <>
struct StructExtends<PhysicalDeviceDescriptorBufferPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceDescriptorBufferDensityMapPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceDescriptorBufferFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<DescriptorBufferBindingPushDescriptorBufferHandleEXT,
                     DescriptorBufferBindingInfoEXT> {
  enum { value = true };
};

template <>
struct StructExtends<OpaqueCaptureDescriptorDataCreateInfoEXT,
                     BufferCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceGraphicsPipelineLibraryPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<GraphicsPipelineLibraryCreateInfoEXT,
                     GraphicsPipelineCreateInfo> {
  enum { value = true };
};
...
//=== VK_AMD_shader_early_and_late_fragment_tests ===
template <>
struct StructExtends<PhysicalDeviceShaderEarlyAndLateFragmentTestsFeaturesAMD,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_KHR_fragment_shader_barycentric ===
template <>
struct StructExtends<PhysicalDeviceFragmentShaderBarycentricFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceFragmentShaderBarycentricPropertiesKHR,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_KHR_shader_subgroup_uniform_control_flow ===
template <>
struct StructExtends<PhysicalDeviceShaderSubgroupUniformControlFlowFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<AccelerationStructureMotionInfoNV,
                     AccelerationStructureCreateInfoKHR> {
  enum { value = true };
};
...
//=== VK_EXT_mesh_shader ===
template <>
struct StructExtends<PhysicalDeviceMeshShaderFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceMeshShaderPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_ycbcr_2plane_444_formats ===
template <>
struct StructExtends<PhysicalDeviceYcbcr2Plane444FormatsFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<CopyCommandTransformInfoQCOM, ImageBlit2> {
  enum { value = true };
};
...
//=== VK_EXT_image_compression_control ===
template <>
struct StructExtends<PhysicalDeviceImageCompressionControlFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<ImageCompressionControlEXT, ImageCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<ImageCompressionControlEXT,
                     PhysicalDeviceImageFormatInfo2> {
  enum { value = true };
};
...
//=== VK_EXT_attachment_feedback_loop_layout ===
template <>
struct StructExtends<PhysicalDeviceAttachmentFeedbackLoopLayoutFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_rgba10x6_formats ===
template <>
struct StructExtends<PhysicalDeviceRGBA10X6FormatsFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_vertex_input_dynamic_state ===
template <>
struct StructExtends<PhysicalDeviceVertexInputDynamicStateFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_physical_device_drm ===
template <>
struct StructExtends<PhysicalDeviceDrmPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_device_address_binding_report ===
template <>
struct StructExtends<PhysicalDeviceAddressBindingReportFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<DeviceAddressBindingCallbackDataEXT,
...
//=== VK_EXT_depth_clip_control ===
template <>
struct StructExtends<PhysicalDeviceDepthClipControlFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PipelineViewportDepthClipControlCreateInfoEXT,
                     PipelineViewportStateCreateInfo> {
  enum { value = true };
};
...
//=== VK_HUAWEI_subpass_shading ===
template <>
struct StructExtends<SubpassShadingPipelineCreateInfoHUAWEI,
                     ComputePipelineCreateInfo> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceSubpassShadingFeaturesHUAWEI,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceSubpassShadingPropertiesHUAWEI,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_HUAWEI_invocation_mask ===
template <>
struct StructExtends<PhysicalDeviceInvocationMaskFeaturesHUAWEI,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_NV_external_memory_rdma ===
template <>
struct StructExtends<PhysicalDeviceExternalMemoryRDMAFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_pipeline_properties ===
template <>
struct StructExtends<PhysicalDevicePipelinePropertiesFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_frame_boundary ===
template <>
struct StructExtends<PhysicalDeviceFrameBoundaryFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<FrameBoundaryEXT, SubmitInfo> {
  enum { value = true };
};

template <> struct StructExtends<FrameBoundaryEXT, SubmitInfo2> {
  enum { value = true };
};

template <> struct StructExtends<FrameBoundaryEXT, PresentInfoKHR> {
  enum { value = true };
};

template <> struct StructExtends<FrameBoundaryEXT, BindSparseInfo> {
  enum { value = true };
};

//=== VK_EXT_multisampled_render_to_single_sampled ===
template <>
struct StructExtends<PhysicalDeviceMultisampledRenderToSingleSampledFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<MultisampledRenderToSingleSampledInfoEXT,
                     SubpassDescription2> {
  enum { value = true };
};
...
//=== VK_EXT_extended_dynamic_state2 ===
template <>
struct StructExtends<PhysicalDeviceExtendedDynamicState2FeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_color_write_enable ===
template <>
struct StructExtends<PhysicalDeviceColorWriteEnableFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PipelineColorWriteCreateInfoEXT,
                     PipelineColorBlendStateCreateInfo> {
  enum { value = true };
};
...
//=== VK_EXT_primitives_generated_query ===
template <>
struct StructExtends<PhysicalDevicePrimitivesGeneratedQueryFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_KHR_ray_tracing_maintenance1 ===
template <>
struct StructExtends<PhysicalDeviceRayTracingMaintenance1FeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_image_view_min_lod ===
template <>
struct StructExtends<PhysicalDeviceImageViewMinLodFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_multi_draw ===
template <>
struct StructExtends<PhysicalDeviceMultiDrawFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceMultiDrawPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_image_2d_view_of_3d ===
template <>
struct StructExtends<PhysicalDeviceImage2DViewOf3DFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_shader_tile_image ===
template <>
struct StructExtends<PhysicalDeviceShaderTileImageFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceShaderTileImagePropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_opacity_micromap ===
template <>
struct StructExtends<PhysicalDeviceOpacityMicromapFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceOpacityMicromapPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<AccelerationStructureTrianglesOpacityMicromapEXT,
                     AccelerationStructureGeometryTrianglesDataKHR> {
  enum { value = true };
};
...
//=== VK_NV_displacement_micromap ===
template <>
struct StructExtends<PhysicalDeviceDisplacementMicromapFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceDisplacementMicromapPropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<AccelerationStructureTrianglesDisplacementMicromapNV,
                     AccelerationStructureGeometryTrianglesDataKHR> {
  enum { value = true };
};
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
...
template <>
struct StructExtends<PhysicalDeviceClusterCullingShaderPropertiesHUAWEI,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceClusterCullingShaderVrsFeaturesHUAWEI,
...
//=== VK_EXT_border_color_swizzle ===
template <>
struct StructExtends<PhysicalDeviceBorderColorSwizzleFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<SamplerBorderColorComponentMappingCreateInfoEXT,
                     SamplerCreateInfo> {
  enum { value = true };
};
...
//=== VK_EXT_pageable_device_local_memory ===
template <>
struct StructExtends<PhysicalDevicePageableDeviceLocalMemoryFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_ARM_shader_core_properties ===
template <>
struct StructExtends<PhysicalDeviceShaderCorePropertiesARM,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_ARM_scheduling_controls ===
template <>
struct StructExtends<DeviceQueueShaderCoreControlCreateInfoARM,
                     DeviceQueueCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceSchedulingControlsFeaturesARM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceSchedulingControlsPropertiesARM,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_image_sliced_view_of_3d ===
template <>
struct StructExtends<PhysicalDeviceImageSlicedViewOf3DFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_VALVE_descriptor_set_host_mapping ===
template <>
struct StructExtends<PhysicalDeviceDescriptorSetHostMappingFeaturesVALVE,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_depth_clamp_zero_one ===
template <>
struct StructExtends<PhysicalDeviceDepthClampZeroOneFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceRenderPassStripedPropertiesARM,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <> struct StructExtends<RenderPassStripeBeginInfoARM, RenderingInfo> {
  enum { value = true };
};
...
//=== VK_QCOM_fragment_density_map_offset ===
template <>
struct StructExtends<PhysicalDeviceFragmentDensityMapOffsetFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceFragmentDensityMapOffsetPropertiesQCOM,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<SubpassFragmentDensityMapOffsetEndInfoQCOM,
                     SubpassEndInfo> {
  enum { value = true };
};
...
//=== VK_NV_copy_memory_indirect ===
template <>
struct StructExtends<PhysicalDeviceCopyMemoryIndirectFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceCopyMemoryIndirectPropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_NV_memory_decompression ===
template <>
struct StructExtends<PhysicalDeviceMemoryDecompressionFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceMemoryDecompressionPropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_NV_device_generated_commands_compute ===
template <>
struct StructExtends<PhysicalDeviceDeviceGeneratedCommandsComputeFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_NV_linear_color_attachment ===
template <>
struct StructExtends<PhysicalDeviceLinearColorAttachmentFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_image_compression_control_swapchain ===
template <>
struct StructExtends<PhysicalDeviceImageCompressionControlSwapchainFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceImageProcessingFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceImageProcessingPropertiesQCOM,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_nested_command_buffer ===
template <>
struct StructExtends<PhysicalDeviceNestedCommandBufferFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceNestedCommandBufferPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_extended_dynamic_state3 ===
template <>
struct StructExtends<PhysicalDeviceExtendedDynamicState3FeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceExtendedDynamicState3PropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
template <>
struct StructExtends<RenderPassSubpassFeedbackCreateInfoEXT,
                     SubpassDescription2> {
  enum { value = true };
};
...
//=== VK_EXT_shader_module_identifier ===
template <>
struct StructExtends<PhysicalDeviceShaderModuleIdentifierFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceShaderModuleIdentifierPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PipelineShaderStageModuleIdentifierCreateInfoEXT,
                     PipelineShaderStageCreateInfo> {
  enum { value = true };
};
...
//=== VK_NV_optical_flow ===
template <>
struct StructExtends<PhysicalDeviceOpticalFlowFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceOpticalFlowPropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<OpticalFlowImageFormatInfoNV,
                     PhysicalDeviceImageFormatInfo2> {
  enum { value = true };
};
...
template <>
struct StructExtends<OpticalFlowSessionCreatePrivateDataInfoNV,
...
//=== VK_EXT_legacy_dithering ===
template <>
struct StructExtends<PhysicalDeviceLegacyDitheringFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_pipeline_protected_access ===
template <>
struct StructExtends<PhysicalDevicePipelineProtectedAccessFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_ANDROID_external_format_resolve ===
template <>
struct StructExtends<PhysicalDeviceExternalFormatResolveFeaturesANDROID,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceExternalFormatResolvePropertiesANDROID,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<AndroidHardwareBufferFormatResolvePropertiesANDROID,
                     AndroidHardwareBufferPropertiesANDROID> {
  enum { value = true };
};
#endif /*VK_USE_PLATFORM_ANDROID_KHR*/
...
//=== VK_KHR_maintenance5 ===
template <>
struct StructExtends<PhysicalDeviceMaintenance5FeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceMaintenance5PropertiesKHR,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PipelineCreateFlags2CreateInfoKHR,
                     ComputePipelineCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<BufferUsageFlags2CreateInfoKHR,
...
//=== VK_KHR_ray_tracing_position_fetch ===
template <>
struct StructExtends<PhysicalDeviceRayTracingPositionFetchFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_shader_object ===
template <>
struct StructExtends<PhysicalDeviceShaderObjectFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceShaderObjectPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_QCOM_tile_properties ===
template <>
struct StructExtends<PhysicalDeviceTilePropertiesFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_SEC_amigo_profiling ===
template <>
struct StructExtends<PhysicalDeviceAmigoProfilingFeaturesSEC,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<AmigoProfilingSubmitInfoSEC, SubmitInfo> {
  enum { value = true };
};

//=== VK_QCOM_multiview_per_view_viewports ===
template <>
struct StructExtends<PhysicalDeviceMultiviewPerViewViewportsFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_NV_ray_tracing_invocation_reorder ===
template <>
struct StructExtends<PhysicalDeviceRayTracingInvocationReorderPropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceRayTracingInvocationReorderFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_NV_extended_sparse_address_space ===
template <>
struct StructExtends<PhysicalDeviceExtendedSparseAddressSpaceFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceExtendedSparseAddressSpacePropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_mutable_descriptor_type ===
template <>
struct StructExtends<PhysicalDeviceMutableDescriptorTypeFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<MutableDescriptorTypeCreateInfoEXT,
                     DescriptorSetLayoutCreateInfo> {
  enum { value = true };
};
...
//=== VK_ARM_shader_core_builtins ===
template <>
struct StructExtends<PhysicalDeviceShaderCoreBuiltinsFeaturesARM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceShaderCoreBuiltinsPropertiesARM,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_EXT_pipeline_library_group_handles ===
template <>
struct StructExtends<PhysicalDevicePipelineLibraryGroupHandlesFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_EXT_dynamic_rendering_unused_attachments ===
template <>
struct StructExtends<PhysicalDeviceDynamicRenderingUnusedAttachmentsFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_NV_low_latency2 ===
template <> struct StructExtends<LatencySubmissionPresentIdNV, SubmitInfo> {
  enum { value = true };
};

template <> struct StructExtends<LatencySubmissionPresentIdNV, SubmitInfo2> {
  enum { value = true };
};
...
//=== VK_KHR_cooperative_matrix ===
template <>
struct StructExtends<PhysicalDeviceCooperativeMatrixFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceCooperativeMatrixPropertiesKHR,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_QCOM_multiview_per_view_render_areas ===
template <>
struct StructExtends<PhysicalDeviceMultiviewPerViewRenderAreasFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<MultiviewPerViewRenderAreasRenderPassBeginInfoQCOM,
                     RenderPassBeginInfo> {
  enum { value = true };
};
...
//=== VK_KHR_video_maintenance1 ===
template <>
struct StructExtends<PhysicalDeviceVideoMaintenance1FeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <> struct StructExtends<VideoInlineQueryInfoKHR, VideoDecodeInfoKHR> {
  enum { value = true };
};

template <> struct StructExtends<VideoInlineQueryInfoKHR, VideoEncodeInfoKHR> {
  enum { value = true };
};

//=== VK_NV_per_stage_descriptor_set ===
template <>
struct StructExtends<PhysicalDevicePerStageDescriptorSetFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_QCOM_image_processing2 ===
template <>
struct StructExtends<PhysicalDeviceImageProcessing2FeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceImageProcessing2PropertiesQCOM,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_QCOM_filter_cubic_weights ===
template <>
struct StructExtends<PhysicalDeviceCubicWeightsFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_QCOM_ycbcr_degamma ===
template <>
struct StructExtends<PhysicalDeviceYcbcrDegammaFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<SamplerYcbcrConversionYcbcrDegammaCreateInfoQCOM,
...
//=== VK_QCOM_filter_cubic_clamp ===
template <>
struct StructExtends<PhysicalDeviceCubicClampFeaturesQCOM,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_KHR_vertex_attribute_divisor ===
template <>
struct StructExtends<PhysicalDeviceVertexAttributeDivisorPropertiesKHR,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PipelineVertexInputDivisorStateCreateInfoKHR,
                     PipelineVertexInputStateCreateInfo> {
  enum { value = true };
};

template <>
struct StructExtends<PhysicalDeviceVertexAttributeDivisorFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_QNX_external_memory_screen_buffer ===
template <>
struct StructExtends<ScreenBufferFormatPropertiesQNX,
...
template <> struct StructExtends<ExternalFormatQNX, ImageCreateInfo> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceExternalMemoryScreenBufferFeaturesQNX,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
//=== VK_MSFT_layered_driver ===
template <>
struct StructExtends<PhysicalDeviceLayeredDriverPropertiesMSFT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_KHR_maintenance6 ===
template <>
struct StructExtends<PhysicalDeviceMaintenance6FeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceMaintenance6PropertiesKHR,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <> struct StructExtends<BindMemoryStatusKHR, BindBufferMemoryInfo> {
  enum { value = true };
};

template <> struct StructExtends<BindMemoryStatusKHR, BindImageMemoryInfo> {
  enum { value = true };
};

//=== VK_NV_descriptor_pool_overallocation ===
template <>
struct StructExtends<PhysicalDeviceDescriptorPoolOverallocationFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
public:
  ...
  PFN_vkGetExecutionGraphPipelineScratchSizeAMDX
      vkGetExecutionGraphPipelineScratchSizeAMDX = 0;
  PFN_vkGetExecutionGraphPipelineNodeIndexAMDX
      vkGetExecutionGraphPipelineNodeIndexAMDX = 0;
  PFN_vkCmdInitializeGraphScratchMemoryAMDX
      vkCmdInitializeGraphScratchMemoryAMDX = 0;
...
public:
  ...
  void init(VULKAN_HPP_NAMESPACE::Instance instanceCpp) VULKAN_HPP_NOEXCEPT {
    ...
    vkCreatePrivateDataSlot = PFN_vkCreatePrivateDataSlot(
        vkGetInstanceProcAddr(instance, "vkCreatePrivateDataSlot"));
    vkDestroyPrivateDataSlot = PFN_vkDestroyPrivateDataSlot(
        vkGetInstanceProcAddr(instance, "vkDestroyPrivateDataSlot"));
    vkSetPrivateData = PFN_vkSetPrivateData(
        vkGetInstanceProcAddr(instance, "vkSetPrivateData"));
    vkGetPrivateData = PFN_vkGetPrivateData(
        vkGetInstanceProcAddr(instance, "vkGetPrivateData"));
    vkCmdSetEvent2 =
        PFN_vkCmdSetEvent2(vkGetInstanceProcAddr(instance, "vkCmdSetEvent2"));
    vkCmdResetEvent2 = PFN_vkCmdResetEvent2(
        vkGetInstanceProcAddr(instance, "vkCmdResetEvent2"));
    vkCmdWaitEvents2 = PFN_vkCmdWaitEvents2(
        vkGetInstanceProcAddr(instance, "vkCmdWaitEvents2"));
    ...
    vkCmdWriteTimestamp2 = PFN_vkCmdWriteTimestamp2(
        vkGetInstanceProcAddr(instance, "vkCmdWriteTimestamp2"));
    vkQueueSubmit2 =
        PFN_vkQueueSubmit2(vkGetInstanceProcAddr(instance, "vkQueueSubmit2"));
    vkCmdCopyBuffer2 = PFN_vkCmdCopyBuffer2(
        vkGetInstanceProcAddr(instance, "vkCmdCopyBuffer2"));
    vkCmdCopyImage2 =
        PFN_vkCmdCopyImage2(vkGetInstanceProcAddr(instance, "vkCmdCopyImage2"));
    vkCmdCopyBufferToImage2 = PFN_vkCmdCopyBufferToImage2(
        vkGetInstanceProcAddr(instance, "vkCmdCopyBufferToImage2"));
    vkCmdCopyImageToBuffer2 = PFN_vkCmdCopyImageToBuffer2(
        vkGetInstanceProcAddr(instance, "vkCmdCopyImageToBuffer2"));
    vkCmdBlitImage2 =
        PFN_vkCmdBlitImage2(vkGetInstanceProcAddr(instance, "vkCmdBlitImage2"));
    vkCmdResolveImage2 = PFN_vkCmdResolveImage2(
        vkGetInstanceProcAddr(instance, "vkCmdResolveImage2"));
    ...
    vkCmdSetCullMode = PFN_vkCmdSetCullMode(
        vkGetInstanceProcAddr(instance, "vkCmdSetCullMode"));
    vkCmdSetFrontFace = PFN_vkCmdSetFrontFace(
        vkGetInstanceProcAddr(instance, "vkCmdSetFrontFace"));
    vkCmdSetPrimitiveTopology = PFN_vkCmdSetPrimitiveTopology(
        vkGetInstanceProcAddr(instance, "vkCmdSetPrimitiveTopology"));
    vkCmdSetViewportWithCount = PFN_vkCmdSetViewportWithCount(
        vkGetInstanceProcAddr(instance, "vkCmdSetViewportWithCount"));
    vkCmdSetScissorWithCount = PFN_vkCmdSetScissorWithCount(
        vkGetInstanceProcAddr(instance, "vkCmdSetScissorWithCount"));
    vkCmdBindVertexBuffers2 = PFN_vkCmdBindVertexBuffers2(
        vkGetInstanceProcAddr(instance, "vkCmdBindVertexBuffers2"));
    vkCmdSetDepthTestEnable = PFN_vkCmdSetDepthTestEnable(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthTestEnable"));
    vkCmdSetDepthWriteEnable = PFN_vkCmdSetDepthWriteEnable(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthWriteEnable"));
    vkCmdSetDepthCompareOp = PFN_vkCmdSetDepthCompareOp(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthCompareOp"));
    vkCmdSetDepthBoundsTestEnable = PFN_vkCmdSetDepthBoundsTestEnable(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthBoundsTestEnable"));
    vkCmdSetStencilTestEnable = PFN_vkCmdSetStencilTestEnable(
        vkGetInstanceProcAddr(instance, "vkCmdSetStencilTestEnable"));
    vkCmdSetStencilOp = PFN_vkCmdSetStencilOp(
        vkGetInstanceProcAddr(instance, "vkCmdSetStencilOp"));
    vkCmdSetRasterizerDiscardEnable = PFN_vkCmdSetRasterizerDiscardEnable(
        vkGetInstanceProcAddr(instance, "vkCmdSetRasterizerDiscardEnable"));
    vkCmdSetDepthBiasEnable = PFN_vkCmdSetDepthBiasEnable(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthBiasEnable"));
    vkCmdSetPrimitiveRestartEnable = PFN_vkCmdSetPrimitiveRestartEnable(
        vkGetInstanceProcAddr(instance, "vkCmdSetPrimitiveRestartEnable"));
    vkGetDeviceBufferMemoryRequirements =
        PFN_vkGetDeviceBufferMemoryRequirements(vkGetInstanceProcAddr(
    ...
    vkCreateVideoSessionKHR = PFN_vkCreateVideoSessionKHR(
        vkGetInstanceProcAddr(instance, "vkCreateVideoSessionKHR"));
    vkDestroyVideoSessionKHR = PFN_vkDestroyVideoSessionKHR(
        vkGetInstanceProcAddr(instance, "vkDestroyVideoSessionKHR"));
    vkGetVideoSessionMemoryRequirementsKHR =
        PFN_vkGetVideoSessionMemoryRequirementsKHR(vkGetInstanceProcAddr(
            instance, "vkGetVideoSessionMemoryRequirementsKHR"));
    vkBindVideoSessionMemoryKHR = PFN_vkBindVideoSessionMemoryKHR(
        vkGetInstanceProcAddr(instance, "vkBindVideoSessionMemoryKHR"));
    vkCreateVideoSessionParametersKHR = PFN_vkCreateVideoSessionParametersKHR(
        vkGetInstanceProcAddr(instance, "vkCreateVideoSessionParametersKHR"));
    vkUpdateVideoSessionParametersKHR = PFN_vkUpdateVideoSessionParametersKHR(
        vkGetInstanceProcAddr(instance, "vkUpdateVideoSessionParametersKHR"));
    ...
    vkCmdBeginVideoCodingKHR = PFN_vkCmdBeginVideoCodingKHR(
        vkGetInstanceProcAddr(instance, "vkCmdBeginVideoCodingKHR"));
    vkCmdEndVideoCodingKHR = PFN_vkCmdEndVideoCodingKHR(
        vkGetInstanceProcAddr(instance, "vkCmdEndVideoCodingKHR"));
    vkCmdControlVideoCodingKHR = PFN_vkCmdControlVideoCodingKHR(
        vkGetInstanceProcAddr(instance, "vkCmdControlVideoCodingKHR"));

    //=== VK_KHR_video_decode_queue ===
    vkCmdDecodeVideoKHR = PFN_vkCmdDecodeVideoKHR(
        vkGetInstanceProcAddr(instance, "vkCmdDecodeVideoKHR"));
    ...
    vkCreateCuModuleNVX = PFN_vkCreateCuModuleNVX(
        vkGetInstanceProcAddr(instance, "vkCreateCuModuleNVX"));
    vkCreateCuFunctionNVX = PFN_vkCreateCuFunctionNVX(
        vkGetInstanceProcAddr(instance, "vkCreateCuFunctionNVX"));
    vkDestroyCuModuleNVX = PFN_vkDestroyCuModuleNVX(
        vkGetInstanceProcAddr(instance, "vkDestroyCuModuleNVX"));
    vkDestroyCuFunctionNVX = PFN_vkDestroyCuFunctionNVX(
        vkGetInstanceProcAddr(instance, "vkDestroyCuFunctionNVX"));
    vkCmdCuLaunchKernelNVX = PFN_vkCmdCuLaunchKernelNVX(
        vkGetInstanceProcAddr(instance, "vkCmdCuLaunchKernelNVX"));
    ...
    vkCreateExecutionGraphPipelinesAMDX =
        PFN_vkCreateExecutionGraphPipelinesAMDX(vkGetInstanceProcAddr(
            instance, "vkCreateExecutionGraphPipelinesAMDX"));
    vkGetExecutionGraphPipelineScratchSizeAMDX =
        PFN_vkGetExecutionGraphPipelineScratchSizeAMDX(vkGetInstanceProcAddr(
            instance, "vkGetExecutionGraphPipelineScratchSizeAMDX"));
    vkGetExecutionGraphPipelineNodeIndexAMDX =
        PFN_vkGetExecutionGraphPipelineNodeIndexAMDX(vkGetInstanceProcAddr(
            instance, "vkGetExecutionGraphPipelineNodeIndexAMDX"));
    vkCmdInitializeGraphScratchMemoryAMDX =
        PFN_vkCmdInitializeGraphScratchMemoryAMDX(vkGetInstanceProcAddr(
            instance, "vkCmdInitializeGraphScratchMemoryAMDX"));
    vkCmdDispatchGraphAMDX = PFN_vkCmdDispatchGraphAMDX(
        vkGetInstanceProcAddr(instance, "vkCmdDispatchGraphAMDX"));
    vkCmdDispatchGraphIndirectAMDX = PFN_vkCmdDispatchGraphIndirectAMDX(
        vkGetInstanceProcAddr(instance, "vkCmdDispatchGraphIndirectAMDX"));
    vkCmdDispatchGraphIndirectCountAMDX =
        PFN_vkCmdDispatchGraphIndirectCountAMDX(vkGetInstanceProcAddr(
            instance, "vkCmdDispatchGraphIndirectCountAMDX"));
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
    ...
    vkCmdSetExclusiveScissorEnableNV = PFN_vkCmdSetExclusiveScissorEnableNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetExclusiveScissorEnableNV"));
    ...
    vkCopyImageToImageEXT = PFN_vkCopyImageToImageEXT(
        vkGetInstanceProcAddr(instance, "vkCopyImageToImageEXT"));
    ...
    vkGetImageSubresourceLayout2EXT = PFN_vkGetImageSubresourceLayout2EXT(
        vkGetInstanceProcAddr(instance, "vkGetImageSubresourceLayout2EXT"));
    if (!vkGetImageSubresourceLayout2KHR)
      vkGetImageSubresourceLayout2KHR = vkGetImageSubresourceLayout2EXT;

    //=== VK_KHR_map_memory2 ===
    vkMapMemory2KHR =
        PFN_vkMapMemory2KHR(vkGetInstanceProcAddr(instance, "vkMapMemory2KHR"));
    vkUnmapMemory2KHR = PFN_vkUnmapMemory2KHR(
        vkGetInstanceProcAddr(instance, "vkUnmapMemory2KHR"));
    ...
    vkCmdSetDepthBias2EXT = PFN_vkCmdSetDepthBias2EXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthBias2EXT"));
    ...
    vkGetEncodedVideoSessionParametersKHR =
        PFN_vkGetEncodedVideoSessionParametersKHR(vkGetInstanceProcAddr(
            instance, "vkGetEncodedVideoSessionParametersKHR"));
    vkCmdEncodeVideoKHR = PFN_vkCmdEncodeVideoKHR(
        vkGetInstanceProcAddr(instance, "vkCmdEncodeVideoKHR"));

#if defined(VK_ENABLE_BETA_EXTENSIONS)
    //=== VK_NV_cuda_kernel_launch ===
    vkCreateCudaModuleNV = PFN_vkCreateCudaModuleNV(
        vkGetInstanceProcAddr(instance, "vkCreateCudaModuleNV"));
    vkGetCudaModuleCacheNV = PFN_vkGetCudaModuleCacheNV(
        vkGetInstanceProcAddr(instance, "vkGetCudaModuleCacheNV"));
    vkCreateCudaFunctionNV = PFN_vkCreateCudaFunctionNV(
        vkGetInstanceProcAddr(instance, "vkCreateCudaFunctionNV"));
    vkDestroyCudaModuleNV = PFN_vkDestroyCudaModuleNV(
        vkGetInstanceProcAddr(instance, "vkDestroyCudaModuleNV"));
    vkDestroyCudaFunctionNV = PFN_vkDestroyCudaFunctionNV(
        vkGetInstanceProcAddr(instance, "vkDestroyCudaFunctionNV"));
    vkCmdCudaLaunchKernelNV = PFN_vkCmdCudaLaunchKernelNV(
        vkGetInstanceProcAddr(instance, "vkCmdCudaLaunchKernelNV"));
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
    ...
    vkGetDescriptorSetLayoutBindingOffsetEXT =
        PFN_vkGetDescriptorSetLayoutBindingOffsetEXT(vkGetInstanceProcAddr(
    ...
    vkCmdBindDescriptorBuffersEXT = PFN_vkCmdBindDescriptorBuffersEXT(
        vkGetInstanceProcAddr(instance, "vkCmdBindDescriptorBuffersEXT"));
    vkCmdSetDescriptorBufferOffsetsEXT = PFN_vkCmdSetDescriptorBufferOffsetsEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetDescriptorBufferOffsetsEXT"));
    vkCmdBindDescriptorBufferEmbeddedSamplersEXT =
        PFN_vkCmdBindDescriptorBufferEmbeddedSamplersEXT(vkGetInstanceProcAddr(
            instance, "vkCmdBindDescriptorBufferEmbeddedSamplersEXT"));
    vkGetBufferOpaqueCaptureDescriptorDataEXT =
        PFN_vkGetBufferOpaqueCaptureDescriptorDataEXT(vkGetInstanceProcAddr(
            instance, "vkGetBufferOpaqueCaptureDescriptorDataEXT"));
    vkGetImageOpaqueCaptureDescriptorDataEXT =
        PFN_vkGetImageOpaqueCaptureDescriptorDataEXT(vkGetInstanceProcAddr(
            instance, "vkGetImageOpaqueCaptureDescriptorDataEXT"));
    vkGetImageViewOpaqueCaptureDescriptorDataEXT =
        PFN_vkGetImageViewOpaqueCaptureDescriptorDataEXT(vkGetInstanceProcAddr(
            instance, "vkGetImageViewOpaqueCaptureDescriptorDataEXT"));
    vkGetSamplerOpaqueCaptureDescriptorDataEXT =
        PFN_vkGetSamplerOpaqueCaptureDescriptorDataEXT(vkGetInstanceProcAddr(
    ...
    vkCmdDrawMeshTasksIndirectEXT = PFN_vkCmdDrawMeshTasksIndirectEXT(
        vkGetInstanceProcAddr(instance, "vkCmdDrawMeshTasksIndirectEXT"));
    vkCmdDrawMeshTasksIndirectCountEXT = PFN_vkCmdDrawMeshTasksIndirectCountEXT(
        vkGetInstanceProcAddr(instance, "vkCmdDrawMeshTasksIndirectCountEXT"));
    ...
    vkGetDeviceFaultInfoEXT = PFN_vkGetDeviceFaultInfoEXT(
        vkGetInstanceProcAddr(instance, "vkGetDeviceFaultInfoEXT"));
    ...
    vkCmdSetVertexInputEXT = PFN_vkCmdSetVertexInputEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetVertexInputEXT"));
    ...
    vkCmdBindInvocationMaskHUAWEI = PFN_vkCmdBindInvocationMaskHUAWEI(
        vkGetInstanceProcAddr(instance, "vkCmdBindInvocationMaskHUAWEI"));

    //=== VK_NV_external_memory_rdma ===
    vkGetMemoryRemoteAddressNV = PFN_vkGetMemoryRemoteAddressNV(
        vkGetInstanceProcAddr(instance, "vkGetMemoryRemoteAddressNV"));
    ...
    vkCmdSetPatchControlPointsEXT = PFN_vkCmdSetPatchControlPointsEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetPatchControlPointsEXT"));
    ...
    vkCmdSetDepthBiasEnableEXT = PFN_vkCmdSetDepthBiasEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthBiasEnableEXT"));
    if (!vkCmdSetDepthBiasEnable)
      vkCmdSetDepthBiasEnable = vkCmdSetDepthBiasEnableEXT;
    vkCmdSetLogicOpEXT = PFN_vkCmdSetLogicOpEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetLogicOpEXT"));
    vkCmdSetPrimitiveRestartEnableEXT = PFN_vkCmdSetPrimitiveRestartEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetPrimitiveRestartEnableEXT"));
    ...
    vkCmdTraceRaysIndirect2KHR = PFN_vkCmdTraceRaysIndirect2KHR(
        vkGetInstanceProcAddr(instance, "vkCmdTraceRaysIndirect2KHR"));

    //=== VK_EXT_multi_draw ===
    vkCmdDrawMultiEXT = PFN_vkCmdDrawMultiEXT(
        vkGetInstanceProcAddr(instance, "vkCmdDrawMultiEXT"));
    vkCmdDrawMultiIndexedEXT = PFN_vkCmdDrawMultiIndexedEXT(
        vkGetInstanceProcAddr(instance, "vkCmdDrawMultiIndexedEXT"));

    //=== VK_EXT_opacity_micromap ===
    vkCreateMicromapEXT = PFN_vkCreateMicromapEXT(
        vkGetInstanceProcAddr(instance, "vkCreateMicromapEXT"));
    vkDestroyMicromapEXT = PFN_vkDestroyMicromapEXT(
        vkGetInstanceProcAddr(instance, "vkDestroyMicromapEXT"));
    vkCmdBuildMicromapsEXT = PFN_vkCmdBuildMicromapsEXT(
        vkGetInstanceProcAddr(instance, "vkCmdBuildMicromapsEXT"));
    vkBuildMicromapsEXT = PFN_vkBuildMicromapsEXT(
        vkGetInstanceProcAddr(instance, "vkBuildMicromapsEXT"));
    vkCopyMicromapEXT = PFN_vkCopyMicromapEXT(
        vkGetInstanceProcAddr(instance, "vkCopyMicromapEXT"));
    vkCopyMicromapToMemoryEXT = PFN_vkCopyMicromapToMemoryEXT(
        vkGetInstanceProcAddr(instance, "vkCopyMicromapToMemoryEXT"));
    vkCopyMemoryToMicromapEXT = PFN_vkCopyMemoryToMicromapEXT(
        vkGetInstanceProcAddr(instance, "vkCopyMemoryToMicromapEXT"));
    ...
    vkCmdCopyMicromapEXT = PFN_vkCmdCopyMicromapEXT(
        vkGetInstanceProcAddr(instance, "vkCmdCopyMicromapEXT"));
    ...
    vkCmdCopyMemoryToMicromapEXT = PFN_vkCmdCopyMemoryToMicromapEXT(
        vkGetInstanceProcAddr(instance, "vkCmdCopyMemoryToMicromapEXT"));
    vkCmdWriteMicromapsPropertiesEXT = PFN_vkCmdWriteMicromapsPropertiesEXT(
        vkGetInstanceProcAddr(instance, "vkCmdWriteMicromapsPropertiesEXT"));
    vkGetDeviceMicromapCompatibilityEXT =
        PFN_vkGetDeviceMicromapCompatibilityEXT(vkGetInstanceProcAddr(
            instance, "vkGetDeviceMicromapCompatibilityEXT"));
    vkGetMicromapBuildSizesEXT = PFN_vkGetMicromapBuildSizesEXT(
        vkGetInstanceProcAddr(instance, "vkGetMicromapBuildSizesEXT"));
    ...
    vkCmdDrawClusterIndirectHUAWEI = PFN_vkCmdDrawClusterIndirectHUAWEI(
        vkGetInstanceProcAddr(instance, "vkCmdDrawClusterIndirectHUAWEI"));

    //=== VK_EXT_pageable_device_local_memory ===
    vkSetDeviceMemoryPriorityEXT = PFN_vkSetDeviceMemoryPriorityEXT(
        vkGetInstanceProcAddr(instance, "vkSetDeviceMemoryPriorityEXT"));

    //=== VK_KHR_maintenance4 ===
    vkGetDeviceBufferMemoryRequirementsKHR =
        PFN_vkGetDeviceBufferMemoryRequirementsKHR(vkGetInstanceProcAddr(
            instance, "vkGetDeviceBufferMemoryRequirementsKHR"));
    if (!vkGetDeviceBufferMemoryRequirements)
      vkGetDeviceBufferMemoryRequirements =
          vkGetDeviceBufferMemoryRequirementsKHR;
    vkGetDeviceImageMemoryRequirementsKHR =
        PFN_vkGetDeviceImageMemoryRequirementsKHR(vkGetInstanceProcAddr(
            instance, "vkGetDeviceImageMemoryRequirementsKHR"));
    if (!vkGetDeviceImageMemoryRequirements)
      vkGetDeviceImageMemoryRequirements =
          vkGetDeviceImageMemoryRequirementsKHR;
    vkGetDeviceImageSparseMemoryRequirementsKHR =
        PFN_vkGetDeviceImageSparseMemoryRequirementsKHR(vkGetInstanceProcAddr(
    ...
    vkGetDescriptorSetLayoutHostMappingInfoVALVE =
        PFN_vkGetDescriptorSetLayoutHostMappingInfoVALVE(vkGetInstanceProcAddr(
    ...
    vkGetDescriptorSetHostMappingVALVE = PFN_vkGetDescriptorSetHostMappingVALVE(
        vkGetInstanceProcAddr(instance, "vkGetDescriptorSetHostMappingVALVE"));

    //=== VK_NV_copy_memory_indirect ===
    vkCmdCopyMemoryIndirectNV = PFN_vkCmdCopyMemoryIndirectNV(
        vkGetInstanceProcAddr(instance, "vkCmdCopyMemoryIndirectNV"));
    vkCmdCopyMemoryToImageIndirectNV = PFN_vkCmdCopyMemoryToImageIndirectNV(
        vkGetInstanceProcAddr(instance, "vkCmdCopyMemoryToImageIndirectNV"));

    //=== VK_NV_memory_decompression ===
    vkCmdDecompressMemoryNV = PFN_vkCmdDecompressMemoryNV(
        vkGetInstanceProcAddr(instance, "vkCmdDecompressMemoryNV"));
    vkCmdDecompressMemoryIndirectCountNV =
        PFN_vkCmdDecompressMemoryIndirectCountNV(vkGetInstanceProcAddr(
            instance, "vkCmdDecompressMemoryIndirectCountNV"));

    //=== VK_NV_device_generated_commands_compute ===
    vkGetPipelineIndirectMemoryRequirementsNV =
        PFN_vkGetPipelineIndirectMemoryRequirementsNV(vkGetInstanceProcAddr(
            instance, "vkGetPipelineIndirectMemoryRequirementsNV"));
    vkCmdUpdatePipelineIndirectBufferNV =
        PFN_vkCmdUpdatePipelineIndirectBufferNV(vkGetInstanceProcAddr(
            instance, "vkCmdUpdatePipelineIndirectBufferNV"));
    vkGetPipelineIndirectDeviceAddressNV =
        PFN_vkGetPipelineIndirectDeviceAddressNV(vkGetInstanceProcAddr(
    ...
    vkCmdSetDepthClampEnableEXT = PFN_vkCmdSetDepthClampEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthClampEnableEXT"));
    vkCmdSetPolygonModeEXT = PFN_vkCmdSetPolygonModeEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetPolygonModeEXT"));
    vkCmdSetRasterizationSamplesEXT = PFN_vkCmdSetRasterizationSamplesEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetRasterizationSamplesEXT"));
    vkCmdSetSampleMaskEXT = PFN_vkCmdSetSampleMaskEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetSampleMaskEXT"));
    ...
    vkCmdSetAlphaToOneEnableEXT = PFN_vkCmdSetAlphaToOneEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetAlphaToOneEnableEXT"));
    vkCmdSetLogicOpEnableEXT = PFN_vkCmdSetLogicOpEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetLogicOpEnableEXT"));
    vkCmdSetColorBlendEnableEXT = PFN_vkCmdSetColorBlendEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetColorBlendEnableEXT"));
    vkCmdSetColorBlendEquationEXT = PFN_vkCmdSetColorBlendEquationEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetColorBlendEquationEXT"));
    vkCmdSetColorWriteMaskEXT = PFN_vkCmdSetColorWriteMaskEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetColorWriteMaskEXT"));
    vkCmdSetRasterizationStreamEXT = PFN_vkCmdSetRasterizationStreamEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetRasterizationStreamEXT"));
    vkCmdSetConservativeRasterizationModeEXT =
        PFN_vkCmdSetConservativeRasterizationModeEXT(vkGetInstanceProcAddr(
            instance, "vkCmdSetConservativeRasterizationModeEXT"));
    vkCmdSetExtraPrimitiveOverestimationSizeEXT =
        PFN_vkCmdSetExtraPrimitiveOverestimationSizeEXT(vkGetInstanceProcAddr(
            instance, "vkCmdSetExtraPrimitiveOverestimationSizeEXT"));
    vkCmdSetDepthClipEnableEXT = PFN_vkCmdSetDepthClipEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthClipEnableEXT"));
    vkCmdSetSampleLocationsEnableEXT = PFN_vkCmdSetSampleLocationsEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetSampleLocationsEnableEXT"));
    vkCmdSetColorBlendAdvancedEXT = PFN_vkCmdSetColorBlendAdvancedEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetColorBlendAdvancedEXT"));
    vkCmdSetProvokingVertexModeEXT = PFN_vkCmdSetProvokingVertexModeEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetProvokingVertexModeEXT"));
    vkCmdSetLineRasterizationModeEXT = PFN_vkCmdSetLineRasterizationModeEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetLineRasterizationModeEXT"));
    vkCmdSetLineStippleEnableEXT = PFN_vkCmdSetLineStippleEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetLineStippleEnableEXT"));
    vkCmdSetDepthClipNegativeOneToOneEXT =
        PFN_vkCmdSetDepthClipNegativeOneToOneEXT(vkGetInstanceProcAddr(
            instance, "vkCmdSetDepthClipNegativeOneToOneEXT"));
    vkCmdSetViewportWScalingEnableNV = PFN_vkCmdSetViewportWScalingEnableNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetViewportWScalingEnableNV"));
    vkCmdSetViewportSwizzleNV = PFN_vkCmdSetViewportSwizzleNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetViewportSwizzleNV"));
    ...
    vkCmdSetCoverageToColorLocationNV = PFN_vkCmdSetCoverageToColorLocationNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetCoverageToColorLocationNV"));
    vkCmdSetCoverageModulationModeNV = PFN_vkCmdSetCoverageModulationModeNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetCoverageModulationModeNV"));
    vkCmdSetCoverageModulationTableEnableNV =
        PFN_vkCmdSetCoverageModulationTableEnableNV(vkGetInstanceProcAddr(
            instance, "vkCmdSetCoverageModulationTableEnableNV"));
    vkCmdSetCoverageModulationTableNV = PFN_vkCmdSetCoverageModulationTableNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetCoverageModulationTableNV"));
    vkCmdSetShadingRateImageEnableNV = PFN_vkCmdSetShadingRateImageEnableNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetShadingRateImageEnableNV"));
    vkCmdSetRepresentativeFragmentTestEnableNV =
        PFN_vkCmdSetRepresentativeFragmentTestEnableNV(vkGetInstanceProcAddr(
            instance, "vkCmdSetRepresentativeFragmentTestEnableNV"));
    vkCmdSetCoverageReductionModeNV = PFN_vkCmdSetCoverageReductionModeNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetCoverageReductionModeNV"));

    //=== VK_EXT_shader_module_identifier ===
    vkGetShaderModuleIdentifierEXT = PFN_vkGetShaderModuleIdentifierEXT(
        vkGetInstanceProcAddr(instance, "vkGetShaderModuleIdentifierEXT"));
    vkGetShaderModuleCreateInfoIdentifierEXT =
        PFN_vkGetShaderModuleCreateInfoIdentifierEXT(vkGetInstanceProcAddr(
            instance, "vkGetShaderModuleCreateInfoIdentifierEXT"));

    //=== VK_NV_optical_flow ===
    vkGetPhysicalDeviceOpticalFlowImageFormatsNV =
        PFN_vkGetPhysicalDeviceOpticalFlowImageFormatsNV(vkGetInstanceProcAddr(
            instance, "vkGetPhysicalDeviceOpticalFlowImageFormatsNV"));
    vkCreateOpticalFlowSessionNV = PFN_vkCreateOpticalFlowSessionNV(
        vkGetInstanceProcAddr(instance, "vkCreateOpticalFlowSessionNV"));
    vkDestroyOpticalFlowSessionNV = PFN_vkDestroyOpticalFlowSessionNV(
        vkGetInstanceProcAddr(instance, "vkDestroyOpticalFlowSessionNV"));
    vkBindOpticalFlowSessionImageNV = PFN_vkBindOpticalFlowSessionImageNV(
        vkGetInstanceProcAddr(instance, "vkBindOpticalFlowSessionImageNV"));
    vkCmdOpticalFlowExecuteNV = PFN_vkCmdOpticalFlowExecuteNV(
        vkGetInstanceProcAddr(instance, "vkCmdOpticalFlowExecuteNV"));

    //=== VK_KHR_maintenance5 ===
    vkCmdBindIndexBuffer2KHR = PFN_vkCmdBindIndexBuffer2KHR(
        vkGetInstanceProcAddr(instance, "vkCmdBindIndexBuffer2KHR"));
    vkGetRenderingAreaGranularityKHR = PFN_vkGetRenderingAreaGranularityKHR(
        vkGetInstanceProcAddr(instance, "vkGetRenderingAreaGranularityKHR"));
    ...
    vkGetImageSubresourceLayout2KHR = PFN_vkGetImageSubresourceLayout2KHR(
        vkGetInstanceProcAddr(instance, "vkGetImageSubresourceLayout2KHR"));
    ...
    vkGetShaderBinaryDataEXT = PFN_vkGetShaderBinaryDataEXT(
        vkGetInstanceProcAddr(instance, "vkGetShaderBinaryDataEXT"));
    vkCmdBindShadersEXT = PFN_vkCmdBindShadersEXT(
        vkGetInstanceProcAddr(instance, "vkCmdBindShadersEXT"));

    //=== VK_QCOM_tile_properties ===
    vkGetFramebufferTilePropertiesQCOM = PFN_vkGetFramebufferTilePropertiesQCOM(
        vkGetInstanceProcAddr(instance, "vkGetFramebufferTilePropertiesQCOM"));
    vkGetDynamicRenderingTilePropertiesQCOM =
        PFN_vkGetDynamicRenderingTilePropertiesQCOM(vkGetInstanceProcAddr(
            instance, "vkGetDynamicRenderingTilePropertiesQCOM"));

    //=== VK_NV_low_latency2 ===
    vkSetLatencySleepModeNV = PFN_vkSetLatencySleepModeNV(
        vkGetInstanceProcAddr(instance, "vkSetLatencySleepModeNV"));
    vkLatencySleepNV = PFN_vkLatencySleepNV(
        vkGetInstanceProcAddr(instance, "vkLatencySleepNV"));
    vkSetLatencyMarkerNV = PFN_vkSetLatencyMarkerNV(
        vkGetInstanceProcAddr(instance, "vkSetLatencyMarkerNV"));
    vkGetLatencyTimingsNV = PFN_vkGetLatencyTimingsNV(
        vkGetInstanceProcAddr(instance, "vkGetLatencyTimingsNV"));
    vkQueueNotifyOutOfBandNV = PFN_vkQueueNotifyOutOfBandNV(
        vkGetInstanceProcAddr(instance, "vkQueueNotifyOutOfBandNV"));
    ...
    vkCmdSetAttachmentFeedbackLoopEnableEXT =
        PFN_vkCmdSetAttachmentFeedbackLoopEnableEXT(vkGetInstanceProcAddr(
            instance, "vkCmdSetAttachmentFeedbackLoopEnableEXT"));

#if defined(VK_USE_PLATFORM_SCREEN_QNX)
    //=== VK_QNX_external_memory_screen_buffer ===
    vkGetScreenBufferPropertiesQNX = PFN_vkGetScreenBufferPropertiesQNX(
        vkGetInstanceProcAddr(instance, "vkGetScreenBufferPropertiesQNX"));
#endif /*VK_USE_PLATFORM_SCREEN_QNX*/
    ...
    vkGetCalibratedTimestampsKHR = PFN_vkGetCalibratedTimestampsKHR(
        vkGetInstanceProcAddr(instance, "vkGetCalibratedTimestampsKHR"));

    //=== VK_KHR_maintenance6 ===
    vkCmdBindDescriptorSets2KHR = PFN_vkCmdBindDescriptorSets2KHR(
        vkGetInstanceProcAddr(instance, "vkCmdBindDescriptorSets2KHR"));
    vkCmdPushConstants2KHR = PFN_vkCmdPushConstants2KHR(
        vkGetInstanceProcAddr(instance, "vkCmdPushConstants2KHR"));
    vkCmdPushDescriptorSet2KHR = PFN_vkCmdPushDescriptorSet2KHR(
        vkGetInstanceProcAddr(instance, "vkCmdPushDescriptorSet2KHR"));
    vkCmdPushDescriptorSetWithTemplate2KHR =
        PFN_vkCmdPushDescriptorSetWithTemplate2KHR(vkGetInstanceProcAddr(
            instance, "vkCmdPushDescriptorSetWithTemplate2KHR"));
    vkCmdSetDescriptorBufferOffsets2EXT =
        PFN_vkCmdSetDescriptorBufferOffsets2EXT(vkGetInstanceProcAddr(
            instance, "vkCmdSetDescriptorBufferOffsets2EXT"));
    vkCmdBindDescriptorBufferEmbeddedSamplers2EXT =
        PFN_vkCmdBindDescriptorBufferEmbeddedSamplers2EXT(vkGetInstanceProcAddr(
  ...
  void init(VULKAN_HPP_NAMESPACE::Device deviceCpp) VULKAN_HPP_NOEXCEPT {
    ...
    //=== VK_VERSION_1_3 ===
    vkCreatePrivateDataSlot = PFN_vkCreatePrivateDataSlot(
        vkGetDeviceProcAddr(device, "vkCreatePrivateDataSlot"));
    vkDestroyPrivateDataSlot = PFN_vkDestroyPrivateDataSlot(
        vkGetDeviceProcAddr(device, "vkDestroyPrivateDataSlot"));
    vkSetPrivateData =
        PFN_vkSetPrivateData(vkGetDeviceProcAddr(device, "vkSetPrivateData"));
    vkGetPrivateData =
        PFN_vkGetPrivateData(vkGetDeviceProcAddr(device, "vkGetPrivateData"));
    vkCmdSetEvent2 =
        PFN_vkCmdSetEvent2(vkGetDeviceProcAddr(device, "vkCmdSetEvent2"));
    vkCmdResetEvent2 =
        PFN_vkCmdResetEvent2(vkGetDeviceProcAddr(device, "vkCmdResetEvent2"));
    vkCmdWaitEvents2 =
        PFN_vkCmdWaitEvents2(vkGetDeviceProcAddr(device, "vkCmdWaitEvents2"));
    vkCmdPipelineBarrier2 = PFN_vkCmdPipelineBarrier2(
    ...
    vkCmdCopyBuffer2 =
        PFN_vkCmdCopyBuffer2(vkGetDeviceProcAddr(device, "vkCmdCopyBuffer2"));
    vkCmdCopyImage2 =
        PFN_vkCmdCopyImage2(vkGetDeviceProcAddr(device, "vkCmdCopyImage2"));
    vkCmdCopyBufferToImage2 = PFN_vkCmdCopyBufferToImage2(
        vkGetDeviceProcAddr(device, "vkCmdCopyBufferToImage2"));
    vkCmdCopyImageToBuffer2 = PFN_vkCmdCopyImageToBuffer2(
        vkGetDeviceProcAddr(device, "vkCmdCopyImageToBuffer2"));
    vkCmdBlitImage2 =
        PFN_vkCmdBlitImage2(vkGetDeviceProcAddr(device, "vkCmdBlitImage2"));
    vkCmdResolveImage2 = PFN_vkCmdResolveImage2(
        vkGetDeviceProcAddr(device, "vkCmdResolveImage2"));
    vkCmdBeginRendering = PFN_vkCmdBeginRendering(
        vkGetDeviceProcAddr(device, "vkCmdBeginRendering"));
    vkCmdEndRendering =
        PFN_vkCmdEndRendering(vkGetDeviceProcAddr(device, "vkCmdEndRendering"));
    vkCmdSetCullMode =
        PFN_vkCmdSetCullMode(vkGetDeviceProcAddr(device, "vkCmdSetCullMode"));
    vkCmdSetFrontFace =
        PFN_vkCmdSetFrontFace(vkGetDeviceProcAddr(device, "vkCmdSetFrontFace"));
    vkCmdSetPrimitiveTopology = PFN_vkCmdSetPrimitiveTopology(
        vkGetDeviceProcAddr(device, "vkCmdSetPrimitiveTopology"));
    vkCmdSetViewportWithCount = PFN_vkCmdSetViewportWithCount(
        vkGetDeviceProcAddr(device, "vkCmdSetViewportWithCount"));
    vkCmdSetScissorWithCount = PFN_vkCmdSetScissorWithCount(
        vkGetDeviceProcAddr(device, "vkCmdSetScissorWithCount"));
    vkCmdBindVertexBuffers2 = PFN_vkCmdBindVertexBuffers2(
        vkGetDeviceProcAddr(device, "vkCmdBindVertexBuffers2"));
    vkCmdSetDepthTestEnable = PFN_vkCmdSetDepthTestEnable(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthTestEnable"));
    vkCmdSetDepthWriteEnable = PFN_vkCmdSetDepthWriteEnable(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthWriteEnable"));
    vkCmdSetDepthCompareOp = PFN_vkCmdSetDepthCompareOp(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthCompareOp"));
    vkCmdSetDepthBoundsTestEnable = PFN_vkCmdSetDepthBoundsTestEnable(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthBoundsTestEnable"));
    vkCmdSetStencilTestEnable = PFN_vkCmdSetStencilTestEnable(
        vkGetDeviceProcAddr(device, "vkCmdSetStencilTestEnable"));
    vkCmdSetStencilOp =
        PFN_vkCmdSetStencilOp(vkGetDeviceProcAddr(device, "vkCmdSetStencilOp"));
    vkCmdSetRasterizerDiscardEnable = PFN_vkCmdSetRasterizerDiscardEnable(
        vkGetDeviceProcAddr(device, "vkCmdSetRasterizerDiscardEnable"));
    vkCmdSetDepthBiasEnable = PFN_vkCmdSetDepthBiasEnable(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthBiasEnable"));
    vkCmdSetPrimitiveRestartEnable = PFN_vkCmdSetPrimitiveRestartEnable(
        vkGetDeviceProcAddr(device, "vkCmdSetPrimitiveRestartEnable"));
    vkGetDeviceBufferMemoryRequirements =
    ...
    vkGetDeviceImageSparseMemoryRequirements =
        PFN_vkGetDeviceImageSparseMemoryRequirements(vkGetDeviceProcAddr(
    ...
    vkGetVideoSessionMemoryRequirementsKHR =
        PFN_vkGetVideoSessionMemoryRequirementsKHR(vkGetDeviceProcAddr(
    ...
    //=== VK_NVX_binary_import ===
    vkCreateCuModuleNVX = PFN_vkCreateCuModuleNVX(
        vkGetDeviceProcAddr(device, "vkCreateCuModuleNVX"));
    vkCreateCuFunctionNVX = PFN_vkCreateCuFunctionNVX(
        vkGetDeviceProcAddr(device, "vkCreateCuFunctionNVX"));
    vkDestroyCuModuleNVX = PFN_vkDestroyCuModuleNVX(
        vkGetDeviceProcAddr(device, "vkDestroyCuModuleNVX"));
    vkDestroyCuFunctionNVX = PFN_vkDestroyCuFunctionNVX(
        vkGetDeviceProcAddr(device, "vkDestroyCuFunctionNVX"));
    vkCmdCuLaunchKernelNVX = PFN_vkCmdCuLaunchKernelNVX(
        vkGetDeviceProcAddr(device, "vkCmdCuLaunchKernelNVX"));
    ...
    vkCmdSetDiscardRectangleEnableEXT = PFN_vkCmdSetDiscardRectangleEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetDiscardRectangleEnableEXT"));
    vkCmdSetDiscardRectangleModeEXT = PFN_vkCmdSetDiscardRectangleModeEXT(
    ...
    vkCreateExecutionGraphPipelinesAMDX =
        PFN_vkCreateExecutionGraphPipelinesAMDX(
            vkGetDeviceProcAddr(device, "vkCreateExecutionGraphPipelinesAMDX"));
    vkGetExecutionGraphPipelineScratchSizeAMDX =
    ...
    vkCmdDispatchGraphAMDX = PFN_vkCmdDispatchGraphAMDX(
        vkGetDeviceProcAddr(device, "vkCmdDispatchGraphAMDX"));
    vkCmdDispatchGraphIndirectAMDX = PFN_vkCmdDispatchGraphIndirectAMDX(
        vkGetDeviceProcAddr(device, "vkCmdDispatchGraphIndirectAMDX"));
    vkCmdDispatchGraphIndirectCountAMDX =
        PFN_vkCmdDispatchGraphIndirectCountAMDX(
            vkGetDeviceProcAddr(device, "vkCmdDispatchGraphIndirectCountAMDX"));
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
    ...
    //=== VK_NV_scissor_exclusive ===
    vkCmdSetExclusiveScissorEnableNV = PFN_vkCmdSetExclusiveScissorEnableNV(
        vkGetDeviceProcAddr(device, "vkCmdSetExclusiveScissorEnableNV"));
    vkCmdSetExclusiveScissorNV = PFN_vkCmdSetExclusiveScissorNV(
    ...
    vkCopyImageToMemoryEXT = PFN_vkCopyImageToMemoryEXT(
        vkGetDeviceProcAddr(device, "vkCopyImageToMemoryEXT"));
    vkCopyImageToImageEXT = PFN_vkCopyImageToImageEXT(
        vkGetDeviceProcAddr(device, "vkCopyImageToImageEXT"));
    vkTransitionImageLayoutEXT = PFN_vkTransitionImageLayoutEXT(
        vkGetDeviceProcAddr(device, "vkTransitionImageLayoutEXT"));
    vkGetImageSubresourceLayout2EXT = PFN_vkGetImageSubresourceLayout2EXT(
        vkGetDeviceProcAddr(device, "vkGetImageSubresourceLayout2EXT"));
    if (!vkGetImageSubresourceLayout2KHR)
      vkGetImageSubresourceLayout2KHR = vkGetImageSubresourceLayout2EXT;

    //=== VK_KHR_map_memory2 ===
    vkMapMemory2KHR =
        PFN_vkMapMemory2KHR(vkGetDeviceProcAddr(device, "vkMapMemory2KHR"));
    vkUnmapMemory2KHR =
        PFN_vkUnmapMemory2KHR(vkGetDeviceProcAddr(device, "vkUnmapMemory2KHR"));
    ...
    //=== VK_EXT_depth_bias_control ===
    vkCmdSetDepthBias2EXT = PFN_vkCmdSetDepthBias2EXT(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthBias2EXT"));
    ...
    //=== VK_KHR_video_encode_queue ===
    vkGetEncodedVideoSessionParametersKHR =
        PFN_vkGetEncodedVideoSessionParametersKHR(vkGetDeviceProcAddr(
    ...
    vkGetCudaModuleCacheNV = PFN_vkGetCudaModuleCacheNV(
        vkGetDeviceProcAddr(device, "vkGetCudaModuleCacheNV"));
    vkCreateCudaFunctionNV = PFN_vkCreateCudaFunctionNV(
    ...
    vkCmdSubpassShadingHUAWEI = PFN_vkCmdSubpassShadingHUAWEI(
        vkGetDeviceProcAddr(device, "vkCmdSubpassShadingHUAWEI"));

    //=== VK_HUAWEI_invocation_mask ===
    vkCmdBindInvocationMaskHUAWEI = PFN_vkCmdBindInvocationMaskHUAWEI(
        vkGetDeviceProcAddr(device, "vkCmdBindInvocationMaskHUAWEI"));

    //=== VK_NV_external_memory_rdma ===
    vkGetMemoryRemoteAddressNV = PFN_vkGetMemoryRemoteAddressNV(
        vkGetDeviceProcAddr(device, "vkGetMemoryRemoteAddressNV"));

    //=== VK_EXT_pipeline_properties ===
    vkGetPipelinePropertiesEXT = PFN_vkGetPipelinePropertiesEXT(
        vkGetDeviceProcAddr(device, "vkGetPipelinePropertiesEXT"));

    //=== VK_EXT_extended_dynamic_state2 ===
    vkCmdSetPatchControlPointsEXT = PFN_vkCmdSetPatchControlPointsEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetPatchControlPointsEXT"));
    vkCmdSetRasterizerDiscardEnableEXT = PFN_vkCmdSetRasterizerDiscardEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetRasterizerDiscardEnableEXT"));
    if (!vkCmdSetRasterizerDiscardEnable)
      vkCmdSetRasterizerDiscardEnable = vkCmdSetRasterizerDiscardEnableEXT;
    vkCmdSetDepthBiasEnableEXT = PFN_vkCmdSetDepthBiasEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthBiasEnableEXT"));
    if (!vkCmdSetDepthBiasEnable)
      vkCmdSetDepthBiasEnable = vkCmdSetDepthBiasEnableEXT;
    vkCmdSetLogicOpEXT = PFN_vkCmdSetLogicOpEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetLogicOpEXT"));
    vkCmdSetPrimitiveRestartEnableEXT = PFN_vkCmdSetPrimitiveRestartEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetPrimitiveRestartEnableEXT"));
    if (!vkCmdSetPrimitiveRestartEnable)
      vkCmdSetPrimitiveRestartEnable = vkCmdSetPrimitiveRestartEnableEXT;
    ...
    //=== VK_EXT_multi_draw ===
    vkCmdDrawMultiEXT =
        PFN_vkCmdDrawMultiEXT(vkGetDeviceProcAddr(device, "vkCmdDrawMultiEXT"));
    vkCmdDrawMultiIndexedEXT = PFN_vkCmdDrawMultiIndexedEXT(
        vkGetDeviceProcAddr(device, "vkCmdDrawMultiIndexedEXT"));

    //=== VK_EXT_opacity_micromap ===
    vkCreateMicromapEXT = PFN_vkCreateMicromapEXT(
        vkGetDeviceProcAddr(device, "vkCreateMicromapEXT"));
    vkDestroyMicromapEXT = PFN_vkDestroyMicromapEXT(
        vkGetDeviceProcAddr(device, "vkDestroyMicromapEXT"));
    vkCmdBuildMicromapsEXT = PFN_vkCmdBuildMicromapsEXT(
        vkGetDeviceProcAddr(device, "vkCmdBuildMicromapsEXT"));
    vkBuildMicromapsEXT = PFN_vkBuildMicromapsEXT(
        vkGetDeviceProcAddr(device, "vkBuildMicromapsEXT"));
    vkCopyMicromapEXT =
        PFN_vkCopyMicromapEXT(vkGetDeviceProcAddr(device, "vkCopyMicromapEXT"));
    vkCopyMicromapToMemoryEXT = PFN_vkCopyMicromapToMemoryEXT(
        vkGetDeviceProcAddr(device, "vkCopyMicromapToMemoryEXT"));
    vkCopyMemoryToMicromapEXT = PFN_vkCopyMemoryToMicromapEXT(
        vkGetDeviceProcAddr(device, "vkCopyMemoryToMicromapEXT"));
    vkWriteMicromapsPropertiesEXT = PFN_vkWriteMicromapsPropertiesEXT(
        vkGetDeviceProcAddr(device, "vkWriteMicromapsPropertiesEXT"));
    vkCmdCopyMicromapEXT = PFN_vkCmdCopyMicromapEXT(
        vkGetDeviceProcAddr(device, "vkCmdCopyMicromapEXT"));
    vkCmdCopyMicromapToMemoryEXT = PFN_vkCmdCopyMicromapToMemoryEXT(
        vkGetDeviceProcAddr(device, "vkCmdCopyMicromapToMemoryEXT"));
    vkCmdCopyMemoryToMicromapEXT = PFN_vkCmdCopyMemoryToMicromapEXT(
        vkGetDeviceProcAddr(device, "vkCmdCopyMemoryToMicromapEXT"));
    vkCmdWriteMicromapsPropertiesEXT = PFN_vkCmdWriteMicromapsPropertiesEXT(
        vkGetDeviceProcAddr(device, "vkCmdWriteMicromapsPropertiesEXT"));
    vkGetDeviceMicromapCompatibilityEXT =
        PFN_vkGetDeviceMicromapCompatibilityEXT(
            vkGetDeviceProcAddr(device, "vkGetDeviceMicromapCompatibilityEXT"));
    vkGetMicromapBuildSizesEXT = PFN_vkGetMicromapBuildSizesEXT(
        vkGetDeviceProcAddr(device, "vkGetMicromapBuildSizesEXT"));

    //=== VK_HUAWEI_cluster_culling_shader ===
    vkCmdDrawClusterHUAWEI = PFN_vkCmdDrawClusterHUAWEI(
        vkGetDeviceProcAddr(device, "vkCmdDrawClusterHUAWEI"));
    vkCmdDrawClusterIndirectHUAWEI = PFN_vkCmdDrawClusterIndirectHUAWEI(
    ...
    if (!vkGetDeviceImageMemoryRequirements)
      vkGetDeviceImageMemoryRequirements =
          vkGetDeviceImageMemoryRequirementsKHR;
    vkGetDeviceImageSparseMemoryRequirementsKHR =
        PFN_vkGetDeviceImageSparseMemoryRequirementsKHR(vkGetDeviceProcAddr(
    ...
    vkGetDescriptorSetHostMappingVALVE = PFN_vkGetDescriptorSetHostMappingVALVE(
        vkGetDeviceProcAddr(device, "vkGetDescriptorSetHostMappingVALVE"));

    //=== VK_NV_copy_memory_indirect ===
    vkCmdCopyMemoryIndirectNV = PFN_vkCmdCopyMemoryIndirectNV(
        vkGetDeviceProcAddr(device, "vkCmdCopyMemoryIndirectNV"));
    vkCmdCopyMemoryToImageIndirectNV = PFN_vkCmdCopyMemoryToImageIndirectNV(
        vkGetDeviceProcAddr(device, "vkCmdCopyMemoryToImageIndirectNV"));

    //=== VK_NV_memory_decompression ===
    vkCmdDecompressMemoryNV = PFN_vkCmdDecompressMemoryNV(
        vkGetDeviceProcAddr(device, "vkCmdDecompressMemoryNV"));
    vkCmdDecompressMemoryIndirectCountNV =
        PFN_vkCmdDecompressMemoryIndirectCountNV(vkGetDeviceProcAddr(
            device, "vkCmdDecompressMemoryIndirectCountNV"));

    //=== VK_NV_device_generated_commands_compute ===
    vkGetPipelineIndirectMemoryRequirementsNV =
        PFN_vkGetPipelineIndirectMemoryRequirementsNV(vkGetDeviceProcAddr(
            device, "vkGetPipelineIndirectMemoryRequirementsNV"));
    vkCmdUpdatePipelineIndirectBufferNV =
        PFN_vkCmdUpdatePipelineIndirectBufferNV(
            vkGetDeviceProcAddr(device, "vkCmdUpdatePipelineIndirectBufferNV"));
    vkGetPipelineIndirectDeviceAddressNV =
        PFN_vkGetPipelineIndirectDeviceAddressNV(vkGetDeviceProcAddr(
            device, "vkGetPipelineIndirectDeviceAddressNV"));
    ...
    vkCmdSetRasterizationStreamEXT = PFN_vkCmdSetRasterizationStreamEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetRasterizationStreamEXT"));
    vkCmdSetConservativeRasterizationModeEXT =
        PFN_vkCmdSetConservativeRasterizationModeEXT(vkGetDeviceProcAddr(
            device, "vkCmdSetConservativeRasterizationModeEXT"));
    vkCmdSetExtraPrimitiveOverestimationSizeEXT =
        PFN_vkCmdSetExtraPrimitiveOverestimationSizeEXT(vkGetDeviceProcAddr(
            device, "vkCmdSetExtraPrimitiveOverestimationSizeEXT"));
    vkCmdSetDepthClipEnableEXT = PFN_vkCmdSetDepthClipEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthClipEnableEXT"));
    vkCmdSetSampleLocationsEnableEXT = PFN_vkCmdSetSampleLocationsEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetSampleLocationsEnableEXT"));
    vkCmdSetColorBlendAdvancedEXT = PFN_vkCmdSetColorBlendAdvancedEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetColorBlendAdvancedEXT"));
    vkCmdSetProvokingVertexModeEXT = PFN_vkCmdSetProvokingVertexModeEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetProvokingVertexModeEXT"));
    vkCmdSetLineRasterizationModeEXT = PFN_vkCmdSetLineRasterizationModeEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetLineRasterizationModeEXT"));
    vkCmdSetLineStippleEnableEXT = PFN_vkCmdSetLineStippleEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetLineStippleEnableEXT"));
    vkCmdSetDepthClipNegativeOneToOneEXT =
        PFN_vkCmdSetDepthClipNegativeOneToOneEXT(vkGetDeviceProcAddr(
            device, "vkCmdSetDepthClipNegativeOneToOneEXT"));
    vkCmdSetViewportWScalingEnableNV = PFN_vkCmdSetViewportWScalingEnableNV(
        vkGetDeviceProcAddr(device, "vkCmdSetViewportWScalingEnableNV"));
    vkCmdSetViewportSwizzleNV = PFN_vkCmdSetViewportSwizzleNV(
        vkGetDeviceProcAddr(device, "vkCmdSetViewportSwizzleNV"));
    vkCmdSetCoverageToColorEnableNV = PFN_vkCmdSetCoverageToColorEnableNV(
        vkGetDeviceProcAddr(device, "vkCmdSetCoverageToColorEnableNV"));
    vkCmdSetCoverageToColorLocationNV = PFN_vkCmdSetCoverageToColorLocationNV(
        vkGetDeviceProcAddr(device, "vkCmdSetCoverageToColorLocationNV"));
    vkCmdSetCoverageModulationModeNV = PFN_vkCmdSetCoverageModulationModeNV(
        vkGetDeviceProcAddr(device, "vkCmdSetCoverageModulationModeNV"));
    vkCmdSetCoverageModulationTableEnableNV =
        PFN_vkCmdSetCoverageModulationTableEnableNV(vkGetDeviceProcAddr(
            device, "vkCmdSetCoverageModulationTableEnableNV"));
    vkCmdSetCoverageModulationTableNV = PFN_vkCmdSetCoverageModulationTableNV(
        vkGetDeviceProcAddr(device, "vkCmdSetCoverageModulationTableNV"));
    vkCmdSetShadingRateImageEnableNV = PFN_vkCmdSetShadingRateImageEnableNV(
        vkGetDeviceProcAddr(device, "vkCmdSetShadingRateImageEnableNV"));
    vkCmdSetRepresentativeFragmentTestEnableNV =
        PFN_vkCmdSetRepresentativeFragmentTestEnableNV(vkGetDeviceProcAddr(
    ...
    //=== VK_EXT_shader_module_identifier ===
    vkGetShaderModuleIdentifierEXT = PFN_vkGetShaderModuleIdentifierEXT(
        vkGetDeviceProcAddr(device, "vkGetShaderModuleIdentifierEXT"));
    vkGetShaderModuleCreateInfoIdentifierEXT =
        PFN_vkGetShaderModuleCreateInfoIdentifierEXT(vkGetDeviceProcAddr(
            device, "vkGetShaderModuleCreateInfoIdentifierEXT"));

    //=== VK_NV_optical_flow ===
    vkCreateOpticalFlowSessionNV = PFN_vkCreateOpticalFlowSessionNV(
        vkGetDeviceProcAddr(device, "vkCreateOpticalFlowSessionNV"));
    vkDestroyOpticalFlowSessionNV = PFN_vkDestroyOpticalFlowSessionNV(
        vkGetDeviceProcAddr(device, "vkDestroyOpticalFlowSessionNV"));
    vkBindOpticalFlowSessionImageNV = PFN_vkBindOpticalFlowSessionImageNV(
        vkGetDeviceProcAddr(device, "vkBindOpticalFlowSessionImageNV"));
    vkCmdOpticalFlowExecuteNV = PFN_vkCmdOpticalFlowExecuteNV(
        vkGetDeviceProcAddr(device, "vkCmdOpticalFlowExecuteNV"));

    //=== VK_KHR_maintenance5 ===
    vkCmdBindIndexBuffer2KHR = PFN_vkCmdBindIndexBuffer2KHR(
        vkGetDeviceProcAddr(device, "vkCmdBindIndexBuffer2KHR"));
    vkGetRenderingAreaGranularityKHR = PFN_vkGetRenderingAreaGranularityKHR(
        vkGetDeviceProcAddr(device, "vkGetRenderingAreaGranularityKHR"));
    vkGetDeviceImageSubresourceLayoutKHR =
        PFN_vkGetDeviceImageSubresourceLayoutKHR(vkGetDeviceProcAddr(
            device, "vkGetDeviceImageSubresourceLayoutKHR"));
    vkGetImageSubresourceLayout2KHR = PFN_vkGetImageSubresourceLayout2KHR(
        vkGetDeviceProcAddr(device, "vkGetImageSubresourceLayout2KHR"));
    ...
    //=== VK_QCOM_tile_properties ===
    vkGetFramebufferTilePropertiesQCOM = PFN_vkGetFramebufferTilePropertiesQCOM(
        vkGetDeviceProcAddr(device, "vkGetFramebufferTilePropertiesQCOM"));
    vkGetDynamicRenderingTilePropertiesQCOM =
        PFN_vkGetDynamicRenderingTilePropertiesQCOM(vkGetDeviceProcAddr(
    ...
    vkLatencySleepNV =
        PFN_vkLatencySleepNV(vkGetDeviceProcAddr(device, "vkLatencySleepNV"));
    vkSetLatencyMarkerNV = PFN_vkSetLatencyMarkerNV(
        vkGetDeviceProcAddr(device, "vkSetLatencyMarkerNV"));
    vkGetLatencyTimingsNV = PFN_vkGetLatencyTimingsNV(
        vkGetDeviceProcAddr(device, "vkGetLatencyTimingsNV"));
    vkQueueNotifyOutOfBandNV = PFN_vkQueueNotifyOutOfBandNV(
        vkGetDeviceProcAddr(device, "vkQueueNotifyOutOfBandNV"));

    //=== VK_EXT_attachment_feedback_loop_dynamic_state ===
    vkCmdSetAttachmentFeedbackLoopEnableEXT =
        PFN_vkCmdSetAttachmentFeedbackLoopEnableEXT(vkGetDeviceProcAddr(
            device, "vkCmdSetAttachmentFeedbackLoopEnableEXT"));

#if defined(VK_USE_PLATFORM_SCREEN_QNX)
    //=== VK_QNX_external_memory_screen_buffer ===
    vkGetScreenBufferPropertiesQNX = PFN_vkGetScreenBufferPropertiesQNX(
        vkGetDeviceProcAddr(device, "vkGetScreenBufferPropertiesQNX"));
#endif /*VK_USE_PLATFORM_SCREEN_QNX*/
    ...
    //=== VK_KHR_maintenance6 ===
    vkCmdBindDescriptorSets2KHR = PFN_vkCmdBindDescriptorSets2KHR(
        vkGetDeviceProcAddr(device, "vkCmdBindDescriptorSets2KHR"));
    vkCmdPushConstants2KHR = PFN_vkCmdPushConstants2KHR(
        vkGetDeviceProcAddr(device, "vkCmdPushConstants2KHR"));
    vkCmdPushDescriptorSet2KHR = PFN_vkCmdPushDescriptorSet2KHR(
        vkGetDeviceProcAddr(device, "vkCmdPushDescriptorSet2KHR"));
    vkCmdPushDescriptorSetWithTemplate2KHR =
        PFN_vkCmdPushDescriptorSetWithTemplate2KHR(vkGetDeviceProcAddr(
    ...
    vkCmdBindDescriptorBufferEmbeddedSamplers2EXT =
        PFN_vkCmdBindDescriptorBufferEmbeddedSamplers2EXT(vkGetDeviceProcAddr(
...
// Compatibility overrides for llama.cpp logging
#include <iostream>
namespace VULKAN_HPP_NAMESPACE {
inline std::ostream &operator<<(std::ostream &os, Buffer const &buffer) {
  return os << (void *)(VkBuffer)buffer;
}
inline std::ostream &operator<<(std::ostream &os, DeviceMemory const &memory) {
  return os << (void *)(VkDeviceMemory)memory;
}
inline std::ostream &operator<<(std::ostream &os, Image const &image) {
  return os << (void *)(VkImage)image;
}
inline std::ostream &operator<<(std::ostream &os, Pipeline const &pipeline) {
  return os << (void *)(VkPipeline)pipeline;
}
} // namespace VULKAN_HPP_NAMESPACE
#endif
